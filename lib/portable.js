...
/**
 * 宿主会话格式代际的文件名（v0 保留裸名 `session.jsonl`，之后是 `session.vN.jsonl`），
 * 与宿主 dsh-session-format/sessionFormatLogFilename 同一规则。
 * 本插件读盘/入包必须按**实际存在的代际**找文件：宿主升到 v3 后写出的是
 * `session.v3.jsonl.zstd`，再按写死的 v0 名字找就是"跳过无 transcript"⇒ 导出丢会话。
 */
export function generationLogName(version, compression) {
  const v = Number.isSafeInteger(version) && version >= 0 ? version : 0
  const base = v === 0 ? 'session.jsonl' : 'session.v' + v + '.jsonl'
  return compression === 'zstd' ? base + '.zstd' : base
}

/** 一个会话目录里所有候选 transcript 文件名（zstd 优先，覆盖 v0..maxVersion 各代际）。 */
export function generationFileCandidates(maxVersion = 9) {
  const out = []
  for (const comp of ['zstd', 'none']) {
    out.push(generationLogName(0, comp))
    for (let v = 1; v <= maxVersion; v++) out.push(generationLogName(v, comp))
  }
  return out
}
