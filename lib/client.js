...
function apply(ctx) {
  ...
  const toneOf = (status) => {
    if (status === 'researching' || status === 'generating' || status === 'crawling') return 'busy'
  ...
  const StudyApp = (props) => {
    ...
    const [confirmDelRemote, setConfirmDelRemote] = React.useState(null)   // 处于「确认删除?」内联态的 remoteGoalId
    ...
    // 导入/拉取是对宿主工作区登记的「带外」改动，连接不会推给浏览器 ⇒ 左栏镜像停在旧快照，
    // 通常要重启才刷新。这里主动把 workspaces/sessions 两个镜像各拉一次，争取免重启让左栏出来。
    // best-effort：宿主 refresh() 语义是私有演进面，拉不动也不报错——拉不出就照旧回落重启提示。
    const refreshWorkspaceMirror = async () => {
      await Promise.all([refreshMirror(workspacesSvc), refreshMirror(sessionsSvc)])
    }
    ...
    // ── 网页课程源（v0.8.0）：抓取预览拉取 + 派发口 ─────────────────────────
    // 抓取中的目标暂停 2.5s 整表轮询：宿主每页都回写 goal.json，整表 refresh 会级联重读
    // 所有目标；进度改由 getCrawlPreview 的 3s 增量轮询驱动（只查展开/可见的抓取中目标）。
    const anyCrawlRunning = goals.some((g) => g.status === 'crawling' || (g.web && g.web.status === 'running'))
    React.useEffect(() => { ui.crawlRunning = anyCrawlRunning }, [anyCrawlRunning])
    React.useEffect(() => {
      if (!open || !anyCrawlRunning) return
      const id = setInterval(() => { refresh() }, 3000)
      return () => clearInterval(id)
    }, [open, anyCrawlRunning, refresh])

    const loadCrawlPreview = React.useCallback(async (goalId) => {
      try {
        const r = await call('study.getCrawlPreview', { goalId })
        if (r && r.ok === true && r.crawled === true) setCrawlPrev((m) => Object.assign({}, m, { [goalId]: r }))
      } catch (e) {}
    }, [])
    React.useEffect(() => {
      const ids = goals.filter((g) => expanded[g.id] === true && g.web && g.web.total > 0 && !crawlPrev[g.id]).map((g) => g.id)
      ids.forEach((id) => { loadCrawlPreview(id) })
    }, [goals, expanded, crawlPrev, loadCrawlPreview])
    React.useEffect(() => {
      if (!open || !anyCrawlRunning) return
      const pull = () => goals.filter((g) => g.status === 'crawling' || (g.web && g.web.status === 'running')).forEach((g) => { loadCrawlPreview(g.id) })
      pull()
      const id = setInterval(pull, 3000)
      return () => clearInterval(id)
    }, [open, anyCrawlRunning, goals, loadCrawlPreview])

    const doStartCrawl = async (g) => {
      const url = String(crawlInputs[g.id] || '').trim()
      if (!url) { setError('请先填写网页地址（http/https 完整网址）'); return }
      setCrawlPrev((m) => { const n = Object.assign({}, m); delete n[g.id]; return n })
      await doAction('crawl:' + g.id, async () => {
        const r = await call('study.startCrawl', { goalId: g.id, url })
        if (r && r.ok === true) loadCrawlPreview(g.id)
        return r
      })
    }

    const doGenerateAll = async (g, regenerate) => {
      await doAction('genall:' + g.id, async () => {
        const r = await call('study.generateAllChapters', { goalId: g.id, regenerate: regenerate || undefined })
        if (r && r.ok === true) setNotice('⚡ 一次性' + (regenerate ? '重新生成' : '生成') + '已派发（' + r.chapters + ' 章）：目标会话按章序撰写，每写完一章自动转「讲义就绪」')
        return r
      })
    }
    ...
    const CRAWL_PAGE_LABEL = { ok: '✓', failed: '✗ 失败', skipped: '⤳ 跳过', blocked: '⛔ robots' }
    const webSection = (g) => {
      const w = g.web || null
      const running = g.status === 'crawling' || (w && w.status === 'running')
      const pv = crawlPrev[g.id]
      const pages = (pv && pv.webCrawl && pv.webCrawl.pages) || []
      const startUrl = (w && w.startUrl) || (pages[0] && pages[0].url) || ''
      const input = React.createElement('input', {
        className: 'stuiInput', value: crawlInputs[g.id] || '', placeholder: '起始网页地址：https://docs.example.com/guide（同域内链自动抓一层，上限 30 页）',
        onChange: (e) => setCrawlInputs((m) => Object.assign({}, m, { [g.id]: e.target.value }))
      })
      if (running) {
        return React.createElement('div', { className: 'stuiDetail' },
          React.createElement('div', { className: 'stuiMeta' }, '🌐 正在抓取网页材料…（' + (w && w.ok || 0) + ' 页成功 / ' + (w && w.failed || 0) + ' 失败 / ' + ((w && w.skipped || 0) + (w && w.blocked || 0)) + ' 跳过，共访问 ' + (w && w.total || pages.length || 0) + '）'),
          pages.length > 0 && React.createElement('div', { className: 'stuiMeta' }, pages.slice(-6).map((p) => React.createElement('div', { key: p.url },
            (CRAWL_PAGE_LABEL[p.status] || p.status) + ' ' + (p.title || p.url) + (p.reason ? '（' + p.reason + '）' : ''))))
        )
      }
      if (!w) {
        return React.createElement('div', { className: 'stuiDetail' },
          ...
          React.createElement('div', { className: 'stuiRow' },
            React.createElement('button', { type: 'button', className: 'stuiAct', disabled: busyKey !== null, onClick: () => doStartCrawl(g) }, busyKey === 'crawl:' + g.id ? '发起中…' : '🌐 抓取网页材料')
          ),
          React.createElement('div', { className: 'stuiMeta' }, '可选：给一个网页地址，插件自动抓取该页及其同域内链作为课程材料；之后「开始调研」将基于材料一次性规划，不再联网漫游。')
        )
      }
      if (w.status === 'done') {
        return React.createElement('div', { className: 'stuiDetail' },
          React.createElement('div', { className: 'stuiMeta' }, '🌐 网页材料 ' + w.ok + ' 页已就绪' + (w.failed ? '（失败 ' + w.failed + '）' : '') + (w.skipped + w.blocked ? '（跳过 ' + (w.skipped + w.blocked) + '）' : '') + ' · 来源 ' + (startUrl || '?') + (pages.some((p) => p.low) ? ' · ⚠ 有页面正文过少（疑似 JS 渲染），可在下方核对后补 URL 重抓' : '')),
          g.status === 'researching' && g.researchDispatched !== true && React.createElement('div', { className: 'stuiRow' },
            React.createElement('button', { type: 'button', className: 'stuiAct', disabled: busyKey !== null, onClick: () => doAction(g.id, () => dispatchResearch(g)) }, busyKey === g.id ? '派发中…' : '✅ 确认材料 → ▶ 开始调研')
          ),
          React.createElement('div', { className: 'stuiRow' },
            input,
            React.createElement('button', { type: 'button', className: 'stuiAct', disabled: busyKey !== null, title: '清空并整批重新抓取', onClick: () => doStartCrawl(g) }, busyKey === 'crawl:' + g.id ? '发起中…' : '🔁 重抓')
          ),
          pv && React.createElement('div', null, pages.map((p, i) => React.createElement('div', { key: p.url + ':' + i, className: 'stuiMeta' },
            (CRAWL_PAGE_LABEL[p.status] || p.status) + ' ' + ((p.title || p.url) + (p.status === 'ok' ? '（' + p.chars + ' 字' + (p.low ? ' ⚠过少' : '') + '）' : p.reason ? '（' + p.reason + '）' : '')).slice(0, 160)))
            )
        )
      }
      return React.createElement('div', { className: 'stuiDetail' },
        React.createElement('div', { className: 'stuiMeta' }, '⚠ 抓取失败：' + ((w && w.error) || '全部页面抓取失败') + ' —— 可修正地址后重抓（抓取明细见「⟳ 刷新明细」）'),
        React.createElement('div', { className: 'stuiRow' },
          input,
          React.createElement('button', { type: 'button', className: 'stuiAct', disabled: busyKey !== null, onClick: () => doStartCrawl(g) }, busyKey === 'crawl:' + g.id ? '发起中…' : '🔁 重新抓取'),
          React.createElement('button', { type: 'button', className: 'stuiAct', disabled: busyKey !== null, onClick: () => loadCrawlPreview(g.id) }, '⟳ 刷新明细')
        ),
        pv && React.createElement('div', null, ((pv.webCrawl && pv.webCrawl.pages) || []).map((p, i) => React.createElement('div', { key: p.url + ':' + i, className: 'stuiMeta' },
          (CRAWL_PAGE_LABEL[p.status] || p.status) + ' ' + String(p.title || p.url).slice(0, 140) + (p.reason ? '（' + p.reason + '）' : ''))))
      )
    }
    ...
    const doConfirmImport = async () => {
      ...
      try {
        ...
        setNotice('✅ ' + (r.idempotent ? '已是最新（无改动）：' : '已导入/更新目标「') + (r.title || r.goalId) + '」' + bits.join(' · ') + '，附件 ' + r.attachments + ' 个。已尝试刷新左栏；若新导入的会话没立刻出现，再重启 DSH 看一次。')
        setImpPreview(null)
        await refresh()
        await refreshWorkspaceMirror()
      } catch (e) {
    ...
    const doSyncPull = (remoteGoalId, discardLocal) => withSyncBusy('pull:' + remoteGoalId, async () => {
      ...
      if (r && r.ok === true) { await refresh(); await refreshWorkspaceMirror() }
    })
    const doSyncDeleteRemote = (remoteGoalId) => withSyncBusy('delremote:' + remoteGoalId, async () => {
      setConfirmDelRemote(null)
      const r = await call('study.syncDeleteRemote', { remoteGoalId: remoteGoalId, confirm: true })
      if (!r || r.ok !== true) { setSyncErr(String((r && r.error) || '删除远端目标失败')); return }
      await loadRemote()
      const lid = (goals.find((g) => g.id === remoteGoalId) || {}).id
      if (lid && inspMap[lid]) await inspectGoal(lid)
    })
        ...
        sync.bound === true && React.createElement(React.Fragment, null,
          ...
          remoteList && React.createElement('div', { className: 'stuiDetail' },
            ...
            (remoteList.goals || []).length === 0 ? React.createElement('div', { className: 'stuiEmpty' }, '（空）同步仓还没有任何目标，首次推送后会出现') :
              (remoteList.goals || []).map((rg) => React.createElement('div', { key: rg.remoteGoalId, className: 'stuiGoal' },
                React.createElement('div', { className: 'stuiChRow' },
                ...
                React.createElement('div', { className: 'stuiChRow' },
                  React.createElement('span', { className: 'stuiMeta' }, fmtWhen(rg.exportedAt)),
                  ...
                  React.createElement('button', { type: 'button', className: 'stuiAct', 'data-tone': 'danger', disabled: syncBusy !== null || noFetch, title: '从仓库删除该目标已推送的数据（本地目标不动，不可逆）', onClick: () => { if (confirmDelRemote === rg.remoteGoalId) doSyncDeleteRemote(rg.remoteGoalId); else setConfirmDelRemote(rg.remoteGoalId) } }, syncBusy === 'delremote:' + rg.remoteGoalId ? '删除中…' : (confirmDelRemote === rg.remoteGoalId ? '确认删除?' : '🗑 删除'))
                )
              ))
          ),
                  ...
                  exp && React.createElement('div', { className: 'stuiGoalBody' },
                    ...
                    React.createElement('div', { className: 'stuiDetail' },
                      ...
                      (g.status === 'approved' || g.status === 'active' || g.status === 'completed') && chapterList.length > 0 && (() => {
                        ...
                        const allReady = pendingN === 0 && genN === 0
                        if (pendingN === 0 && !allReady) return React.createElement('div', { className: 'stuiMeta' }, '⚡ 有章节正在生成中，等它们完成后才可再次一次性生成')
                        return React.createElement('div', { className: 'stuiRow' },
                          React.createElement('button', {
                            type: 'button', className: 'stuiAct', 'data-tone': 'primary', disabled: goalBusy || busyKey === 'genall:' + g.id,
                            title: allReady ? '按章序逐章覆盖重写全部讲义' : '向目标会话派发一条整课指令：按章序逐章写「待生成」章，每写完一章自动转「讲义就绪」',
                            onClick: () => doGenerateAll(g, allReady ? true : undefined)
                          }, busyKey === 'genall:' + g.id ? '派发中…' : (allReady ? '⚡ 重新生成全部讲义' : '⚡ 一次性生成全部讲义（' + pendingN + ' 章）'))
                        )
                      })(),
  ...
  ctx.effect(() => {
    const id = setInterval(() => {
      if (ui.open && activeRefresh && !ui.crawlRunning) activeRefresh()
    }, 2500)
    return () => clearInterval(id)
