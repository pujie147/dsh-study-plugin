  ...
  ctx.inject(['webServer', 'agents', 'workspaceRegistry', 'tools'], (sctx) => {
    ...
    const researchInstruction = async (g, absDir) => {
      ...
      return '你是课程规划 AI。请为「' + g.topic + '」规划一套学习课程（目标水平: ' + (g.target_level || '未说明') + (g.requirements ? '；附加要求: ' + g.requirements : '') + '）。\n'
        ...
        + '3) 把结果以 JSON 写入文件 ' + dir + '/draft.json，结构: {"course":"课程名","overview":"全课程总览: 这个主题解决什么问题 → 核心抽象框架(分层/组件及职责, 如控制层/服务层/数据层) → 章次地图(每章一句话: 它在整体框架中的位置)","chapters":[{"title":"章节标题","summary":"本章学什么/为什么","est_hours":建议学习小时数(可省),"focus_points":[3-6个核心知识点字符串],"env_baseline":"(可选,通常仅第1章)全课程环境与工程基线: 工具链+最小可运行工程+git 仓库(主分支=全课程基线, 后续章一律从合并点开 chapter/NN-主题 独立分支开发, 章末合并回主分支, 禁止直接在主分支开发)","depends_on":[可选,前置章号;前置章标注"作黑盒使用,本章详述"且归本章清偿的概念也列于此],"project_thread":"(可选,编程/工程类主题)本章在统一工程上的增量内容+分支动线(从上一章合并点 checkout, 章末验证点+合并回主分支)"}]}。非工程类主题可整体省略这三个可选字段。若你没有文件写入工具，则把同一 JSON 作为纯文本回复(不要用代码围栏)。\n'
    ...
    async function approveGoal(g) {
      ...
      g.chapters = g.draft.chapters.map((c) => ({
        ...
        env_baseline: c.env_baseline, depends_on: c.depends_on, project_thread: c.project_thread,
      ...
      for (const row of (idx.goals || [])) {
        ...
        out.push({
          ...
          draft: g.draft ? {
            ...
            chapters: Array.isArray(g.draft.chapters) ? g.draft.chapters.map((c) => ({ index: c.index, title: c.title, summary: c.summary, est_hours: c.est_hours === undefined ? null : c.est_hours, focus_points: c.focus_points || [], env_baseline: c.env_baseline, depends_on: c.depends_on || [], project_thread: c.project_thread })) : []
          } : null,
