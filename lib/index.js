...
export function apply(ctx, config) {
  ...
  ctx.inject(['webServer', 'agents', 'workspaceRegistry', 'tools'], (sctx) => {
    ...
    /** 绑定状态机的公共尾段：验证 token → 定位或创建固定仓 → 落盘。失败不落盘（PAT 场景）/保留 auth 待续（设备码场景）。 */
    ...
    /** 固定仓名 + 认领标记：404 建仓 / 有标记采用 / 无标记拒绝 / 422 竞态重走。绝不静默写别人的仓。 */
    async function locateOrCreateRepo(cfg) {
      ...
      if (mk.status === 200 && mk.data && mk.data.content) {
        ...
        if (!j || j.kind !== 'dsh-study-sync') throw new Error('账号 ' + owner + ' 下 ' + GH_REPO_NAME + ' 已存在但不是学习区的同步仓（认领标记不符），不会写入它；请先改名那个仓或换账号')
      } else if (mk.status === 409) {
        // 409 = 仓是空的（刚建/建仓者没推东西）⇒ 首推认领标记即完成初始化
        const msg = b64(JSON.stringify({ kind: 'dsh-study-sync', tool: 'study-plugin', createdBy: owner, createdAt: nowISO() }, null, 2))
        const wp = await ghApi(cfg, 'PUT', path_ + '/contents/' + GH_MARKER_FILE, { message: 'study-plugin: init sync repo', content: msg, branch })
        if (!(wp.status >= 200 && wp.status < 300)) throw new GhError('写入认领标记（token 需要 Contents 写权限）', wp)
      } else if (mk.status === 404) {
        throw new Error('账号 ' + owner + ' 下 ' + GH_REPO_NAME + ' 已存在且有内容，但没有学习区的认领标记 —— 这不是本插件的同步仓，拒绝写入。请把那个仓改名后重试，或换账号')
    ...
    handlers['study.syncGetConfig'] = async () => {
      try {
        const cfg = await loadSyncCfg()
        return stripUndefined({ ok: true, bound: syncBound(cfg), bindState: bindStateOf(cfg), fetch: typeof fetch === 'function', config: redactSyncCfg(cfg) })
    ...
    handlers['study.syncPollDeviceFlow'] = async () => {
      try {
        ...
        const cfg = await loadSyncCfg()
        const r = await ghReq(String(cfg.endpoints.web).replace(/\/+$/, '') + '/login/oauth/device/poll', {
          method: 'POST',
          headers: { 'content-type': 'application/x-www-form-urlencoded', accept: 'application/json', 'user-agent': 'study-plugin' },
          body: new URLSearchParams({ client_id: deviceFlow.clientId, device_code: deviceFlow.deviceCode }).toString()
        })
        const d = r.data || {}
        if (d.error === 'authorization_pending') return stripUndefined({ ok: true, status: 'pending' })
