...
try {
  ...
  // createRequire 的基准必须是**宿主包真实存在**的路径；指向不存在的 lib/package.json
  // 会让解析直接以该文件为起点而不再向上找。依次试仓库根与宿主安装位置。
  const { createRequire } = await import('node:module')
  const bases = [import.meta.url]
  try {
    const { homedir } = await import('node:os')
    const { join } = await import('node:path')
    bases.push(pathToFileURL(join(homedir(), '.dsh', 'profiles', 'web', 'package.json')).href)
    const appdata = (await import('node:process')).env.APPDATA || ''
    if (appdata) bases.push(pathToFileURL(join(appdata, 'npm', 'node_modules', '@deepseek-ai', 'dsh', 'package.json')).href)
  } catch {}
  for (const base of bases) {
    try {
      const resolved = createRequire(base).resolve('@deepseek-ai/dsh-tools')
      defineTool = (await import(pathToFileURL(resolved).href)).defineTool
      break
    } catch {}
  }
  if (!defineTool) defineTool = undefined
}
...
export function apply(ctx, config) {
  ...
  ctx.inject(['webServer', 'agents', 'workspaceRegistry', 'tools'], (sctx) => {
    ...
    const UPLOADS_DIR = BASE + '/uploads'   // Web 上传的临时包（uploadId → <id>.upload + <id>.json sidecar）
    ...
    /** 宿主当前会话格式代际：从 locate() 反推的文件名读（v3 起是 session.vN.jsonl.zstd），不猜。 */
    let _genVersion
    function currentGenerationVersion() {
      if (_genVersion !== undefined) return _genVersion
      try {
        const pp = portableDeps.persistence
        const m = /session(?:\.v(\d+))?\.jsonl/.exec(path.basename(pp.locate({ cwd: BASE, id: 'study-probe' }).path))
        _genVersion = m && m[1] !== undefined ? Number(m[1]) : 0
      } catch { _genVersion = 0 }
      return _genVersion
    }
    /** 宿主 encodeSegment 的逆：`~XXXX`（大写 hex）还原为对应 code unit；`.`/`..` 被特殊转义。 */
    const decodeSegmentName = (seg) => {
      const s = String(seg)
      if (s === '~002E') return '.'
      if (s === '~002E~002E') return '..'
      return s.replace(/~([0-9A-Fa-f]{4})/g, (m, hex) => String.fromCharCode(parseInt(hex, 16)))
    }
    const stampNow = () => {
      const d = new Date()
    ...
    async function globalSessionIndex() {
      ...
      if (pp && typeof pp.list === 'function') {
        try {
          ...
          return { byId, degraded: '', fromHostList: true }
      ...
      // 兜底扫盘只在宿主 list() 不可得/抛错时走。注意：list() 对"读不懂的 header"是**静默跳过**，
      // 它认不到 ≠ id 空闲 ⇒ 只要装的是真后端（有 locate/open），连 list() 成功返回的视图也
      // 必须与目录名扫描取并集（约束 1：duplicate id 会炸整个会话列表）。
      const conservative = !!(pp && typeof pp.locate === 'function' && typeof pp.open === 'function')
      for (const pk of await fsp.readdir(root, { withFileTypes: true }).catch(() => [])) {
        ...
        for (const sd of await fsp.readdir(pdir, { withFileTypes: true }).catch(() => [])) {
          ...
          let h
          for (const cand of P.generationFileCandidates()) {
            h = await readHeaderCheap(path.join(dirAbs, cand))
            if (!h && /\.jsonl$/.test(cand)) { try { h = JSON.parse((await fsp.readFile(path.join(dirAbs, cand), 'utf8')).split('\n').filter(Boolean)[0]) } catch {} }
            if (h) break
          }
          if (h) push(String(h.id || ''), h.cwd, dirAbs)
          else if (conservative) push(String(decodeSegmentName(sd.name)), '', dirAbs)
        }
      }
    ...
    async function collectSessions(absDir, g, warnings) {
      ...
      for (const it of dirs) {
        ...
        // 按**实际存在的代际**找 transcript（宿主升 v3 后文件名是 session.v3.jsonl.zstd，
        // 写死 v0 名字会把所有会话当成"无 transcript"跳过 ⇒ 导出丢会话）。
        let kind = '', bytes = undefined, foundName = ''
        for (const cand of P.generationFileCandidates()) {
          const b = await fsp.readFile(path.join(dirAbs, cand)).catch(() => undefined)
          if (b !== undefined) { kind = /\.zstd$/.test(cand) ? 'zstd' : 'jsonl'; bytes = b; foundName = cand; break }
        }
        if (bytes === undefined) {
        ...
        try {
          ...
          if (live && typeof portableDeps.sessions.flush === 'function') { await portableDeps.sessions.flush(live); bytes = await fsp.readFile(path.join(dirAbs, foundName)) }
    ...
    async function buildGoalBundle(goalId) {
      ...
      for (const s of sessions) {
        ...
        for (const extra of await fsp.readdir(dirAbs).catch(() => [])) {
          if (P.generationFileCandidates().includes(extra)) continue
          const st = await statOpt(path.join(dirAbs, extra))
          if (st && st.isFile() && st.size <= MAX_GOAL_FILE_BYTES) {
    ...
    async function readExport(ref) {
      let abs = ''
      if (ref && ref.upload !== undefined && ref.upload !== '') {
        // Web 上传的临时包：只认不透明 uploadId（防穿越），落点固定在 uploads 目录
        const id = String(ref.upload)
        if (!/^[a-f0-9]{16,32}$/.test(id)) throw new Error('非法 uploadId')
        abs = path.join(UPLOADS_DIR, id + '.upload')
      } else if (ref && ref.path) abs = path.resolve(expandHomePath(ref.path))
      ...
      } else throw new Error('缺少 file（exports 目录内文件名）、path（zip 绝对路径）或 upload（上传返回的 uploadId）')
      const st = await statOpt(abs)
    ...
    // ── Web 上传包（uploads/ 目录：内容 sha256 是唯一性身份，同名不同内容可共存） ──
    const uploadAbs = (id) => path.join(UPLOADS_DIR, id + '.upload')
    const sidecarAbs = (id) => path.join(UPLOADS_DIR, id + '.json')
    const cleanUploadName = (raw) => {
      const base = path.basename(String(raw || '')).replace(/[\\/:*?"<>|\x00-\x1f]/g, '_').trim().slice(0, 180)
      return base || 'upload.zip'
    }
    async function readSidecar(id) {
      try { return JSON.parse(await fsp.readFile(sidecarAbs(id), 'utf8')) } catch { return undefined }
    }
    async function writeSidecar(meta) {
      await fsp.mkdir(UPLOADS_DIR, { recursive: true })
      await fsp.writeFile(sidecarAbs(meta.uploadId), JSON.stringify(meta, null, 2), 'utf8')
    }
    /** 按内容哈希找已存在的同包（去重第三道闸：跨重启也认）。 */
    async function findUploadBySha(sha) {
      const names = await fsp.readdir(UPLOADS_DIR).catch(() => [])
      let best = null
      for (const name of names) {
        if (!/\.json$/.test(name)) continue
        const meta = await readSidecar(name.slice(0, -5)).catch(() => undefined)
        if (!meta || meta.sha256 !== sha) continue
        if (!(await statOpt(uploadAbs(meta.uploadId)))) continue
        if (!best || String(meta.mtimeMs || 0) > String(best.mtimeMs || 0)) best = meta
      }
      return best
    }
    /** 惰性清理：sidecar 与 .upload 一起消失；超过 TTL 的整对删除。无后台定时器。 */
    async function sweepUploads() {
      const names = await fsp.readdir(UPLOADS_DIR).catch(() => [])
      for (const name of names) {
        if (!/\.json$/.test(name)) continue
        const id = name.slice(0, -5)
        if (!/^[a-f0-9]{16,32}$/.test(id)) { try { await fsp.unlink(path.join(UPLOADS_DIR, name)) } catch {} continue }
        const upSt = await statOpt(uploadAbs(id))
        if (!upSt) { try { await fsp.unlink(sidecarAbs(id)) } catch {}; continue }
        const age = Date.now() - upSt.mtimeMs
        if (age > UPLOAD_TTL_MS) {
          try { await fsp.unlink(uploadAbs(id)) } catch {}
          try { await fsp.unlink(sidecarAbs(id)) } catch {}
        }
      }
      // 没有 sidecar 的孤儿 .upload（写入中途崩溃残留）一并清掉
      for (const name of await fsp.readdir(UPLOADS_DIR).catch(() => [])) {
        if (!/\.upload$/.test(name)) continue
        const id = name.slice(0, -'.upload'.length)
        if (!/^[a-f0-9]{16,32}$/.test(id)) { try { await fsp.unlink(path.join(UPLOADS_DIR, name)) } catch {}; continue }
        if (!(await statOpt(sidecarAbs(id))) && Date.now() - ((await statOpt(uploadAbs(id)))?.mtimeMs || 0) > UPLOAD_TTL_MS) {
          try { await fsp.unlink(uploadAbs(id)) } catch {}
        }
      }
    }
    async function uploadRow(id) {
      const meta = await readSidecar(id)
      const st = await statOpt(uploadAbs(id))
      if (!st) return undefined
      return stripUndefined({
        uploadId: id,
        name: (meta && meta.name) || id + '.zip',
        bytes: st.size,
        mtime: st.mtime.toISOString(),
        sha256: (meta && meta.sha256) || '',
        goalId: (meta && meta.goalId) || '',
        title: (meta && meta.title) || '',
      })
    }

    handlers['study.listUploads'] = async () => {
      try {
        await sweepUploads()
        const names = await fsp.readdir(UPLOADS_DIR).catch(() => [])
        const ids = [...new Set(names.filter((n) => /\.upload$/.test(n)).map((n) => n.slice(0, -'.upload'.length)))]
        const out = []
        for (const id of ids) { const row = await uploadRow(id); if (row) out.push(row) }
        out.sort((a, b) => String(b.mtime).localeCompare(String(a.mtime)))
        // 重复展示：同 sha 只留最新一份为主行，其余标 duplicateOf（面板给"删旧留新"入口）
        const bySha = new Map()
        for (const r of out) {
          if (!r.sha256) continue
          const prev = bySha.get(r.sha256)
          if (prev) r.duplicateOf = prev.uploadId
          else bySha.set(r.sha256, r)
        }
        return stripUndefined({ ok: true, dir: UPLOADS_DIR, uploads: out })
      } catch (e) { return { ok: false, error: errText(e) } }
    }

    handlers['study.deleteUpload'] = async (args) => {
      try {
        const id = String((args && args.upload) || '')
        if (!/^[a-f0-9]{16,32}$/.test(id)) return { ok: false, error: '非法 uploadId' }
        const st = await statOpt(uploadAbs(id))
        if (!st) return { ok: false, error: '上传包不存在' }
        await fsp.unlink(uploadAbs(id))
        try { await fsp.unlink(sidecarAbs(id)) } catch {}
        return { ok: true }
      } catch (e) { return { ok: false, error: errText(e) } }
    }

    handlers['study.renameUpload'] = async (args) => {
      try {
        const id = String((args && args.upload) || '')
        if (!/^[a-f0-9]{16,32}$/.test(id)) return { ok: false, error: '非法 uploadId' }
        if (!(await statOpt(uploadAbs(id)))) return { ok: false, error: '上传包不存在' }
        const meta = (await readSidecar(id)) || { uploadId: id }
        meta.name = cleanUploadName((args && args.name) || meta.name)
        await writeSidecar(meta)
        return stripUndefined({ ok: true, name: meta.name })
      } catch (e) { return { ok: false, error: errText(e) } }
    }

    handlers['study.uploadInfo'] = async (args) => {
      try {
        const id = String((args && args.upload) || '')
        if (!/^[a-f0-9]{16,32}$/.test(id)) return { ok: false, error: '非法 uploadId' }
        const row = await uploadRow(id)
        if (!row) return { ok: false, error: '上传包不存在' }
        return stripUndefined({ ok: true, upload: row })
      } catch (e) { return { ok: false, error: errText(e) } }
    }
    ...
    async function resolveImport(manifest, opts) {
      ...
      // 占用登记：宿主 list() 对"读不懂的 header"是**静默跳过**，认不到 ≠ id 空闲 ⇒
      // 真后端（有 locate/open）时还要扫目录名兜底。但**只登记本地读得出的**（header
      // version == 当前代际）：读不出的本地文件宿主本来就拒读，换身份另存副本才是正解
      // （v0→v3 升级过渡期的旧会话就落在这里）。atTarget 的 id 不算 —— 那是原地更新路径。
      const scanOccupied = new Set()
      {
        const rootDir = (() => { try { return path.dirname(projectDirOf(BASE)) } catch { return '' } })()
        const ppLive = portableDeps.persistence
        if (rootDir && ppLive && typeof ppLive.locate === 'function' && typeof ppLive.open === 'function') {
          const genRe = /session(?:\.v(\d+))?\.jsonl/
          for (const pk of await fsp.readdir(rootDir, { withFileTypes: true }).catch(() => [])) {
            if (!pk.isDirectory()) continue
            const pdir = path.join(rootDir, pk.name)
            for (const sd of await fsp.readdir(pdir, { withFileTypes: true }).catch(() => [])) {
              if (!sd.isDirectory()) continue
              // 只有与宿主当前代际一致、且 header.id 与目录名解码值逐字一致的 transcript
              // 才算"本机存在这条会话"（防 ~XX 转义歧义误判；也防把"本地就是包的前缀"
              // 的原地会话错判成别的目录占用）
              let readable = false
              for (const cand of P.generationFileCandidates()) {
                const m = genRe.exec(cand)
                const fileGen = m && m[1] !== undefined ? Number(m[1]) : 0
                if (fileGen !== currentGenerationVersion()) continue
                const buf = await fsp.readFile(path.join(pdir, sd.name, cand)).catch(() => undefined)
                if (buf === undefined || buf.length > MAX_DECODE_BYTES) continue
                let h
                try { h = /\.zstd$/.test(cand) ? P.readSessionHeader(buf) : JSON.parse(buf.toString('utf8').split('\n').filter(Boolean)[0]) } catch { continue }
                if (String(h.id || '') === decodeSegmentName(sd.name)) { readable = true; break }
              }
              if (readable) scanOccupied.add(String(decodeSegmentName(sd.name)))
            }
          }
        }
      }
      ...
      for (const s of (manifest.sessions || [])) {
        ...
        const atTarget = async (id) => {
          for (const o of occOf(id)) {
            try { if (sessionTargetPath(canonicalDir, id) === (o.path || sessionTargetPath(o.cwd || canonicalDir, id))) return true } catch {}
          }
          // 目录名扫描登记的占用给不了 path（宿主 list() 静默跳过它读不懂的 header），
          // 用 locate 反推比对：该 id 按 canonicalDir 落点真有当代际 transcript ⇒ 它就是
          // 本地这条（原地更新路径），不是"别的目录占用"。
          if (scanOccupied.has(id)) {
            try {
              const p = sessionTargetPath(canonicalDir, id)
              for (const cand of P.generationFileCandidates()) if (cand === path.basename(p)) return !!(await statOpt(p))
            } catch {}
          }
          return false
        }
        // 占用判定：宿主 list() 视图里"别的目录占用"，或目录名扫描登记过的 id（含 list()
        // 静默跳过的读不懂 header）。atTarget 的 id 不算 —— 那是"原地更新"路径。
        const occupiedElsewhere = async (id) => !(await atTarget(id)) && (occOf(id).length > 0 || scanOccupied.has(id))
      ...
      async function findContentTwin(cwd, s) {
        ...
        for (const it of await fsp.readdir(pdir, { withFileTypes: true }).catch(() => [])) {
          ...
          let buf
          for (const cand of P.generationFileCandidates()) {
            buf = await fsp.readFile(path.join(pdir, it.name, cand)).catch(() => undefined)
      ...
      const candidates = []
        ...
        for (const c of candidates) {
          if (await occupiedElsewhere(c.id)) { why = 'id ' + c.id + ' 已被别的目录占用（沿用会让宿主 list() 抛重复）'; continue }
          if (occOf(c.id).length && await atTarget(c.id)) { chosen = { id: c.id, identity: 'update' }; break }
    ...
    // ── /study-upload 路由（POST multipart/form-data，流式落盘 uploads/）────────
    // 与 /study-rpc 分开：RPC 有 1MB JSON 上限且要整体驻留内存，zip 包动辄几十 MB。
    // 零依赖手写 multipart 解析：只认第一个文件 part（多个 → 400），其余字段忽略。
    // 唯一性三道闸：① 同请求多文件拒收 ② 并发窗口内 readdir 比 sha256 ③ sidecar 跨重启去重。
    const parseBoundary = (contentType) => {
      const m = /boundary=(?:"([^"]+)"|([^;\s]+))/i.exec(String(contentType || ''))
      const b = m && (m[1] || m[2])
      if (!b || b.length > 70) return undefined
      return Buffer.from('--' + b)
    }
    const findBuf = (hay, needle, from) => hay.indexOf(needle, Math.max(0, from))
    const headerValue = (headersText, name) => {
      const re = new RegExp('^' + name + ':\\s*(.+)$', 'im')
      const m = re.exec(headersText || '')
      return m ? m[1].trim() : ''
    }
    const handleUpload = (req, res) => {
      const failAndCleanup = async (code, msg) => {
        try { res.writeHead(code, { 'content-type': 'text/plain; charset=utf-8' }); res.end(msg) } catch {}
      }
      if (req.method !== 'POST') return failAndCleanup(405, 'method not allowed')
      const boundary = parseBoundary(req.headers && req.headers['content-type'])
      if (!boundary) return failAndCleanup(400, 'expect multipart/form-data with boundary')
      const uploadId = randomBytes(16).toString('hex')
      const tmpPath = uploadAbs(uploadId)
      // 事件监听必须在**同步路径**注册完 —— 宿主可能同步开始投递 data/end。
      // createWriteStream 是惰性的（open 发生在第一次 write），异步建目录不冲突；
      // UPLOADS_DIR 不存在时 open 会 ENOENT ⇒ settle 的 error 路径兜底回 500，不会留残file。
      const ws = createWriteStream(tmpPath)
      fsp.mkdir(UPLOADS_DIR, { recursive: true }).catch(() => {})

      // 解析状态机：preamble → part headers → file body → epilogue
      // （awaitsep = SEP/TERM 已消费、其后的 '--'/'\r\n' 整段跨 chunk，尾巴原样保留等拼接）
      ...
      let fileName = ''
      let failed = ''
      let settled = false    // settle 已进入（幂等守卫）
      let settledDone = false // settle 的清理/响应已全部落定（兜底定时器据此判断）
      const SEP = boundary
      const CRLF = Buffer.from('\r\n')
      const TERM = Buffer.concat([CRLF, SEP])
      // 保留窗口：buf 后缀里"恰是 pat 前缀"的最长串。正文里的孤立 \r/- 不算前缀，
      // 会被正常落盘；跨 chunk 的真分隔符则一定完整留在 buf 里等下一块拼上。
      const suffixPrefixKeep = (b, pat) => {
        for (let k = Math.min(b.length, pat.length - 1); k > 0; k--) {
          if (b.subarray(b.length - k).equals(pat.subarray(0, k))) return k
        }
        return 0
      }
      const afterSepBytes = () => {
        if (buf.length < 2) { state = 'awaitsep'; return }
        if (buf[0] === 45 && buf[1] === 45) { state = 'done'; return }
        if (buf[0] === 13 && buf[1] === 10) { buf = buf.subarray(2); state = 'headers'; return }
        buf = Buffer.alloc(0); state = 'preamble'
      }
      const feed = (chunk) => {
        if (settled) return
        buf = buf.length ? Buffer.concat([buf, chunk]) : chunk
        if (process.env.STUDY_UPLOAD_DEBUG) console.log('DBG feed len=' + chunk.length + ' state=' + state + ' buf=' + buf.length)
        for (;;) {
          if (state === 'preamble') {
            const at = findBuf(buf, SEP, 0)
            if (at < 0) { buf = buf.subarray(Math.max(0, buf.length - suffixPrefixKeep(buf, SEP))); return }
            buf = buf.subarray(at + SEP.length)
            afterSepBytes()
            continue
          }
          if (state === 'awaitsep') { afterSepBytes(); continue }
          if (state === 'headers') {
            const at = findBuf(buf, CRLF + CRLF, 0)
            if (at < 0) { if (buf.length > 16384) { failed = 'part headers too large'; settle(true).catch(() => {}) }; return }
            const head = buf.subarray(0, at).toString('utf8')
            buf = buf.subarray(at + 4)
            const cd = headerValue(head, 'content-disposition')
            const isFile = /filename\s*=/i.test(cd)
            if (isFile) {
              if (fileFound) { failed = 'multipart 里有多个文件 part'; settle(true).catch(() => {}); return }
              fileFound = true
          ...
          if (state === 'skipbody') {
            const at = findBuf(buf, TERM, 0)
            if (at < 0) { buf = buf.subarray(Math.max(0, buf.length - suffixPrefixKeep(buf, TERM))); return }
            buf = buf.subarray(at + TERM.length)
            afterSepBytes()
            continue
          }
          if (state === 'body') {
            const at = findBuf(buf, TERM, 0)
            if (at < 0) {
              // 不变式：进入 body 时 buf 不含 SEP。刷掉前段后若尾巴里出现 SEP，说明该 SEP
              // 与后续字节（-- / CRLF）整段跨了 chunk ⇒ 把它当"半个终止符"整体留住。
              let keep = suffixPrefixKeep(buf, TERM)
              const lastSep = buf.lastIndexOf(SEP)
              if (lastSep >= 0) keep = Math.max(keep, buf.length - lastSep)
              const flushable = buf.length - keep
              if (flushable > 0) {
                const piece = buf.subarray(0, flushable)
                total += piece.length
                if (total > MAX_UPLOAD_BYTES) { failed = 'too large'; try { req.destroy() } catch {}; settle(true).catch(() => {}); return }
                ws.write(piece)
            ...
            total += piece.length
            if (total > MAX_UPLOAD_BYTES) { failed = 'too large'; settle(true).catch(() => {}); return }
            ws.write(piece)
            buf = buf.subarray(at + TERM.length)
            afterSepBytes()
            continue
      ...
      const settle = async (premature) => {
        if (settled) return
        settled = true
        if (process.env.STUDY_UPLOAD_DEBUG) console.log('DBG settle premature=' + String(premature) + ' failed=' + failed + ' state=' + state + ' total=' + total)
        const drain = new Promise((r) => { try { ws.end(r) } catch { r() } })
        // 兜底：夹具/极端情况下 end 回调可能永不落定 ⇒ 30ms 后强制毁句柄；close 事件
        // 最多再等 1s（Windows 上句柄未释放时 unlink 会 EBUSY）。定时器必须 unref ——
        // 否则吊住事件循环，测试进程退不出去。
        let timer = null
        try { timer = setTimeout(() => { try { ws.destroy() } catch {} }, 30) } catch {}
        if (timer && typeof timer.unref === 'function') timer.unref()
        await new Promise((r) => {
          if (ws.destroyed || ws.writableFinished) return r()
          ws.once('close', () => r())
          const t = setTimeout(r, 1000)
          if (typeof t.unref === 'function') t.unref()
        })
        const unlinkRetry = async (target) => {
          for (let i = 0; i < 20; i++) {
            try { await fsp.unlink(target); return } catch (e) {
        ...
        const cleanup = async () => {
          if (premature || failed) { try { ws.destroy() } catch {} }   // 不等未 drain 的 write
          else await drain
          await unlinkRetry(tmpPath)
          await unlinkRetry(sidecarAbs(uploadId))
        }
        if (premature || failed) {
          await cleanup().catch(() => {})
          settledDone = true
          if (failed === 'too large') return failAndCleanup(413, 'upload exceeds ' + MAX_UPLOAD_BYTES + ' B')
          if (/多个文件/.test(failed)) return failAndCleanup(400, failed)
          return failAndCleanup(400, 'malformed multipart: ' + (failed || 'stream ended before final boundary'))
        }
        if (!fileFound) { await cleanup(); settledDone = true; return failAndCleanup(400, 'multipart 里没有文件 part') }
        if (total === 0) { await cleanup(); settledDone = true; return failAndCleanup(400, '空文件') }
        await drain
        const digest = P.sha256Hex(await fsp.readFile(tmpPath))
        // 第二/三道闸：与已有上传包按内容比对（readdir 现算，覆盖并发窗口 + 跨重启）
        const dup = await findUploadBySha(digest)
        if (dup) {
          await unlinkRetry(tmpPath)
          settledDone = true
          return respondJson(res, { ok: true, duplicate: true, uploadId: dup.uploadId, file: dup.name, bytes: dup.bytes || total })
        }
        const meta = { uploadId, name: cleanUploadName(fileName), bytes: total, sha256: digest, mtimeMs: Date.now() }
        await writeSidecar(meta)
        settledDone = true
        return respondJson(res, { ok: true, duplicate: false, uploadId, file: meta.name, bytes: total })
      }

      req.on('data', (c) => { feed(Buffer.isBuffer(c) ? c : Buffer.from(c)) })
      const settleNow = (premature) => {
        settle(premature).catch((e) => console.error('study-upload settle error:', errText(e)))
        // 兜底：失败路径的 ws.destroy() 在夹具/极端情况下可能不发 'close' ⇒ drain 永不落定。
        // 1s 后若仍未落定，直接毁句柄并尽力清残file（幂等守卫保证不会与正常路径打架）。
        setTimeout(() => {
          if (!settledDone) {
            try { ws.destroy() } catch {}
            fsp.unlink(tmpPath).catch(() => {})
            fsp.unlink(sidecarAbs(uploadId)).catch(() => {})
          }
        }, 1000).unref?.()
      }
      req.on('end', () => { settleNow(state !== 'done') })
      req.on('aborted', () => { settleNow(true) })
      req.on('error', () => { settleNow(true) })
    }
    const respondJson = (res, obj) => {
      try { res.writeHead(200, { 'content-type': 'application/json; charset=utf-8' }); res.end(JSON.stringify(obj)) } catch {}
    }

    sctx.effect(() => {
      const dispose = sctx.webServer.register({ kind: 'prefix', path: '/study-upload', handler: handleUpload })
      return () => { if (typeof dispose === 'function') dispose() }
    }, 'study-plugin: /study-upload route')

    // 启动即清一次过期上传包（无后台定时器；运行期由 listUploads 顺带清）
    sctx.effect(() => { sweepUploads().catch(() => {}) }, 'study-plugin: uploads sweep')
