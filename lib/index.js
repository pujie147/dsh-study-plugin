...
export function apply(ctx, config) {
  ...
  // 用户教学偏好记忆体：只记结构/组织/构建规则类习惯（不记课程内容），随导出包旅行。
  const PREFS_PATH = BASE + '/_meta/teaching-prefs.md'
  ...
  ctx.inject(['webServer', 'agents', 'workspaceRegistry', 'tools'], (sctx) => {
    ...
    async function writeJson(absPath, data) {
      ...
      // 原子落盘：D3 的轮询读与抓取期的高频写会撞车，半截 JSON 必须不可能被读到
      const tmp = absPath + '.' + process.pid + '.' + (jsonTmpSeq++) + '.tmp'
    ...
    async function adoptDraftFile(goal) {
      ...
      goal.draft = {
        ...
        chapters: data.chapters.map((c, i) => ({
          ...
          env_baseline: typeof c.env_baseline === 'string' && c.env_baseline.trim() ? c.env_baseline.trim() : undefined,
          depends_on: Array.isArray(c.depends_on) ? c.depends_on.map(Number).filter((n) => Number.isInteger(n) && n >= 1 && n <= i + 1) : undefined,
          project_thread: typeof c.project_thread === 'string' && c.project_thread.trim() ? c.project_thread.trim() : undefined,
          gap_notes: Array.isArray(c.gap_notes) && c.gap_notes.some((x) => String(x || '').trim()) ? c.gap_notes.map((x) => String(x || '').trim()).filter(Boolean) : undefined
        })),
        created_at: nowISO(),
        approved: false,
        lastError: undefined
    ...
    // ══════════════════════════════════════════════════════════════════
    // D36: 测试单元 —— 单元讲义测试 + 目标测试 + 错题本
    // 文件即真相：测试卷（chapters/NN-test-XX.md、goal-test-XX.md）由会话 AI 写出，
    // 插件轮询采纳（与讲义 adoptChapterFiles 同法）；错题本（NN-mistakes.md、
    // goal-mistakes.md）由会话 AI 按 mistakeRuleText 统一格式追加，插件只数 ^## 块。
    // ══════════════════════════════════════════════════════════════════
    const chapterTestFile = (idx, n) => pad2(idx) + '-test-' + pad2(n) + '.md'
    const chapterMistakesFile = (idx) => pad2(idx) + '-mistakes.md'
    const goalTestFile = (n) => 'goal-test-' + pad2(n) + '.md'
    const GOAL_MISTAKES_FILE = 'goal-mistakes.md'

    /** 错题本统一记录规则（章/目标跨来源逐字同源，注入所有相关指令）。 */
    const mistakeRuleText = (mistakesPathAbs) =>
      '错题本记录规则（同一规则适用于章与目标的所有环节）：\n'
      + '文件: ' + mistakesPathAbs + '。每条错题一个块，只追加、绝不改写或删除已有条目；同考点再次答错 ⇒ 新记一块并在块尾标注「重错（第 N 次）」；重做答对 ⇒ 在原条目末尾追加一行「→ 已在 YYYY-MM-DD 重做答对」；条目永久留痕，不销账不删除。\n'
      + '块格式: 「## 考点：<一句话考点> ｜ 错于 YYYY-MM-DD ｜ 来源 <来源文件名或讲义问答>」，块内固定三段: **原题**（完整题面与选项）、**易错点**（学习者当时的错误思路）、**考点讲解**（为什么错 + 考点内容摘要，公式 LaTeX、图 mermaid）。\n'

    const countMistakes = (text) => (String(text || '').match(/^##\s/gm) || []).length

    /** 单份测试文件的采纳（mergeTestEntry 依赖调用方 saveGoal）。 */
    async function mergeTestEntry(owner, listKey, n, file, absPath) {
      owner[listKey] = owner[listKey] || []
      const list = owner[listKey]
      const hit = list.find((t) => Number(t.n) === n)
      const raw = await readTextFile(absPath)
      const hasContent = raw !== undefined && raw.trim().length > 200
      if (!hit) {
        if (!hasContent) return false
        list.push({ n, file, status: 'ready', createdAt: nowISO() })
        return true
      }
      if (hit.status === 'generating' && hasContent) {
        hit.status = 'ready'
    ...
    /**
     * 测试卷轮询采纳：目录里出现但 goal.json 未登记的 NN-test-XX.md / goal-test-XX.md
     * （会话 AI 直接写出的）按 ready 收编；generating 的条目内容达标后转 ready。
     * 派发前先调用 ⇒ 重复点击的编号不会撞上盘上已有文件。
     */
    async function adoptTestFiles(goal) {
      let changed = false
      const absDir = await goalDir(goal)
      let chFiles = []
      try { chFiles = await fsp.readdir(absDir + '/chapters') } catch (e) {}
      for (const f of chFiles) {
        const m = /^(\d{2})-test-(\d{2})\.md$/.exec(f)
        if (!m) continue
        const ch = (goal.chapters || []).find((c) => pad2(c.index) === m[1])
        if (!ch) continue
        if (await mergeTestEntry(ch, 'tests', Number(m[2]), f, absDir + '/chapters/' + f)) changed = true
      }
      let rootFiles = []
      try { rootFiles = await fsp.readdir(absDir) } catch (e) {}
      for (const f of rootFiles) {
        const m = /^goal-test-(\d{2})\.md$/.exec(f)
        if (!m) continue
        if (await mergeTestEntry(goal, 'goalTests', Number(m[1]), f, absDir + '/' + f)) changed = true
      }
      if (changed) {
        goal.updatedAt = nowISO()
        await saveGoal(goal)
    ...
    const prefsWriteBackHint = '偏好沉淀: 若学习者在本次任务沟通中要求长期调整课程结构、组织方式、构建方法或规则(非一次性要求)，提炼成一句话规则，先征得学习者同意，再追加到 ' + PREFS_PATH + '（格式: - YYYY-MM-DD 规则；文件不存在则创建；与已有条目重复或冲突时合并改写而非堆叠；只记规则与结构习惯，不记具体课程内容）。\n'

    async function readTeachingPrefs() {
      try { return (await fsp.readFile(PREFS_PATH, 'utf8')).trim() } catch { return '' }
    }
    const prefsSection = (t) => t ? '用户教学偏好(长期习惯，必须遵守；只涉结构与规则，勿把其中具体课程内容当成知识来源):\n' + t + '\n' : ''

    /** 网页材料清单（材料模式调研的依据）：只认 webCrawl.pages 里 status=ok 且有文件的页。 */
    async function webMaterialOf(g) {
      if (!g || !g.webCrawl || g.webCrawl.status !== 'done') return []
      const absDir = await goalDir(g)
      const out = []
      for (const p of (g.webCrawl.pages || [])) {
        if (p.status !== 'ok' || !p.file) continue
        try {
          const st = await fsp.stat(absDir + '/research/web/' + p.file)
          if (st && st.isFile()) out.push({ file: 'research/web/' + p.file, abs: (absDir + '/research/web/' + p.file).replace(/\\/g, '/'), url: p.url, title: p.title, chars: p.chars })
        } catch (e) {}
      }
      return out
    }

    /** 目标会话的「目标测试」自带能力说明：学习者在会话内开口要卷时不必走面板。 */
    const goalTestCapabilityText = (absDir) => {
      const dir = absDir.replace(/\\/g, '/')
      return '【目标测试能力】此后你在此会话中收到目标测试/模拟卷要求时: 新卷写入 ' + dir + '/goal-test-NN.md（NN=01,02…递增，先 ls 目录取现有最大号+1，绝不复用或覆盖已有卷），错题本 ' + dir + '/goal-mistakes.md。出卷流程: ①只先给蓝图(题型×题量×模块配比+分值)并问学习者确认(要求里已给全题型与题量时复述一遍直接继续) → ②read 错题本按考点/模块归类(文件不存在则跳过) → ③错题按额度入卷: 蓝图每个模块槽位至多取该模块错题数=槽位数(优先「重错」条目)，缺口出新题，落选留待下次，绝不超插蓝图外模块的错题，重做题目标题行标【错题重做｜来源 <原条目>】 → ④落盘(题目在前、全部答案与解析集中在末尾「参考答案与解析」节，每题标注考点；公式 LaTeX、图 mermaid) → ⑤一句话汇报「已生成考卷(共 N 题，含错题重做 X 题)」，并提示到学习区展开「目标测试」点对应卷的「▶ 打开」进入陪练。答错即记错题，遵守同一规则:\n'
        + mistakeRuleText(dir + '/goal-mistakes.md')
    }

    const researchInstruction = async (g, absDir) => {
      const dir = absDir.replace(/\\/g, '/')
      const prefs = await readTeachingPrefs()
      const material = await webMaterialOf(g)
      const step1 = material.length
        ? '步骤: 1) 本次为「网页课程源」调研，材料已抓好，**禁止重新抓取网页**。先用 read 工具逐份读完以下 ' + material.length + ' 份网页材料（禁跳读、禁只凭文件名猜测内容）：\n'
          + material.map((m2) => '  - ' + m2.abs + '（来源 ' + m2.url + '）').join('\n') + '\n'
          + '以材料为课程结构的主要依据；仅当材料覆盖不到的关键知识点缺失时才用 web_search 补充(至多 2 次)；某章存在材料讲不到的知识点时，在该章 JSON 里加可选字段 "gap_notes":["缺什么料、希望补什么"(一句话一条)]。\n'
      ...
      return '你是课程规划 AI。请为「' + g.topic + '」规划一套学习课程（目标水平: ' + (g.target_level || '未说明') + (g.requirements ? '；附加要求: ' + g.requirements : '') + '）。\n'
        + prefsSection(prefs)
        + step1
        + '2) 按主题的自然知识结构(不要按学时)切分为若干章节(通常 5-10 章)，从基础到目标水平递进；编排遵循先总后分: 先立全课程之总(overview)，再逐章展开，每章内部同样先框架后细节。\n'
        + '3) 把结果以 JSON 写入文件 ' + dir + '/draft.json，结构: {"course":"课程名","overview":"全课程总览: 这个主题解决什么问题 → 核心抽象框架(分层/组件及职责, 如控制层/服务层/数据层) → 章次地图(每章一句话: 它在整体框架中的位置)","chapters":[{"title":"章节标题","summary":"本章学什么/为什么","est_hours":建议学习小时数(可省),"focus_points":[3-6个核心知识点字符串],"env_baseline":"(可选,通常仅第1章)全课程环境与工程基线: 工具链+最小可运行工程+git 仓库(主分支=全课程基线, 后续章一律从合并点开 chapter/NN-主题 独立分支开发, 章末合并回主分支, 禁止直接在主分支开发)","depends_on":[可选,前置章号;前置章标注"作黑盒使用,本章详述"且归本章清偿的概念也列于此],"project_thread":"(可选,编程/工程类主题)本章在统一工程上的增量内容+分支动线(从上一章合并点 checkout, 章末验证点+合并回主分支)","gap_notes":[可选,网页材料覆盖不到的知识点,一句话一条]}]}。非工程类主题可整体省略这些可选字段。若你没有文件写入工具，则把同一 JSON 作为纯文本回复(不要用代码围栏)。\n'
        + '4) 排版约定(将随讲义落实): 公式一律 LaTeX($…$/$$…$$)；结构/流程/时序图优先 mermaid。\n'
        + '5) 完成后用一句话回复: 草案已生成(共 N 章)，并列出每章标题。'
        + goalTestCapabilityText(absDir)
        + prefsWriteBackHint
    }
    const retryInstruction = async (g, absDir, reason) => {
      const dir = absDir.replace(/\\/g, '/')
      const prefs = await readTeachingPrefs()
      const material = await webMaterialOf(g)
      const step = material.length
        ? '步骤同前(**材料模式**: 先用 read 逐份读完 ' + (absDir + '/research/web').replace(/\\/g, '/') + '/ 下全部 ' + material.length + ' 份网页材料, 禁止重新抓取网页, web_search 至多 2 次, 缺料章写 gap_notes) → 按自然章节切分(先总后分) → 将 JSON 写入 ' + dir + '/draft.json（结构不变, 含 overview 全课程总览与章节可选字段 env_baseline/depends_on/project_thread/gap_notes；无文件工具则纯文本回复 JSON）。完成后一句话列出章节。'
        : '步骤同前: 联网调研 → 按自然章节切分(先总后分) → 将 JSON 写入 ' + dir + '/draft.json（结构不变, 含 overview 全课程总览与章节可选字段 env_baseline/depends_on/project_thread；无文件工具则纯文本回复 JSON）。完成后一句话列出章节。'
    ...
    // ══════════════════════════════════════════════════════════════════
    // 网页课程源（v0.8.0，docs/design/web-course.md）
    // 宿主半抓取：起始 URL → 同域内链 BFS（深度 2 / 30 页 / 单页 2MB），HTML→纯文本
    // 落 <goal>/research/web/NN-slug.txt + manifest.json；文件即真相（D3），随导出整树旅行。
    // 规则写死（本版不开配置）；robots 仅对起始域、仅认 User-agent: * 段，拉不到=放行。
    // ══════════════════════════════════════════════════════════════════
    const CRAWL_MAX_DEPTH = 2
    const CRAWL_MAX_PAGES = 30
    const CRAWL_MAX_BYTES_PER_PAGE = 2 * 1024 * 1024
    const CRAWL_PER_REQ_TIMEOUT = 15000
    const CRAWL_TOTAL_BUDGET = 120000
    const CRAWL_MIN_CHARS = 200
    const webCrawlActive = new Set()   // goalId：防同目标并发抓取

    const cleanEntityMap = { lt: '<', gt: '>', amp: '&', quot: '"', apos: "'", nbsp: ' ' }
    const decodeEntities = (s) => String(s).replace(/&(#x?[0-9a-fA-F]+|[a-zA-Z]+);/g, (m, body) => {
      if (body[0] === '#') {
        const cp = body[1] === 'x' || body[1] === 'X' ? parseInt(body.slice(2), 16) : parseInt(body.slice(1), 10)
        if (!Number.isInteger(cp) || cp < 0 || cp > 0x10FFFF) return m
        try { return String.fromCodePoint(cp) } catch { return m }
      }
      const k = body.toLowerCase()
      return Object.prototype.hasOwnProperty.call(cleanEntityMap, k) ? cleanEntityMap[k] : m
    })
    const normalizeCrawlUrl = (u) => {
      try {
        const x = new URL(String(u))
        if (x.protocol !== 'http:' && x.protocol !== 'https:') return undefined
        x.hash = ''
        x.hostname = x.hostname.toLowerCase()
        if (x.pathname === '/' && x.search) x.search = ''
        return x.toString()
      } catch { return undefined }
    }
    const hostRoot = (h) => String(h || '').toLowerCase().replace(/^www\./, '')
    const sameSite = (a, b) => hostRoot(a) === hostRoot(b)
    const urlExt = (u) => {
      try { return new URL(u).pathname.match(/\.(html?|php|aspx?|jsp)$/i) ? 'html' : /\.[a-z0-9]{1,8}$/i.test(new URL(u).pathname) ? 'other' : '' }
      catch { return '' }
    }
    const stripCrawlTag = (html, tag) => html.replace(new RegExp('<' + tag + '\\b[^>]*>[\\s\\S]*?</' + tag + '\\s*>', 'gi'), ' ')

    /** HTML → { title, text, links }：剥噪音标签、块级边界转换行、实体解码、空白压缩。 */
    function parseHtmlForCrawl(html, baseUrl) {
      let s = String(html || '')
      const tm = /<title\b[^>]*>([\s\S]*?)<\/title\s*>/i.exec(s)
      const title = tm ? decodeEntities(tm[1].replace(/<[^>]+>/g, '')).replace(/\s+/g, ' ').trim().slice(0, 200) : ''
      s = stripCrawlTag(s, 'script'); s = stripCrawlTag(s, 'style'); s = stripCrawlTag(s, 'noscript')
      s = stripCrawlTag(s, 'template'); s = stripCrawlTag(s, 'svg'); s = stripCrawlTag(s, 'head')
      s = stripCrawlTag(s, 'iframe')
      s = s.replace(/<br\s*\/?>/gi, '\n')
      s = s.replace(/<\/(p|div|li|h[1-6]|tr|td|th|section|article|header|footer|nav|ul|ol|table|blockquote|pre)\s*>/gi, '\n')
      s = s.replace(/<li\b[^>]*>/gi, '\n- ')
    ...
    /** robots.txt 轻量解析：仅认 User-agent 星号 段的 Disallow（路径前缀匹配）；文件缺失/拉不到 = 放行。 */
    function parseRobotsDisallow(text) {
      ...
      for (const line of String(text || '').split(/\r?\n/)) {
        ...
        if (!l) continue
        const uv = /^user-agent\s*:\s*(.*)$/i.exec(l)
        if (uv) { star = uv[1].trim() === '*'; continue }
        if (!star) continue
        const dd = /^disallow\s*:\s*(\S*)$/i.exec(l)
        if (dd && dd[1]) out.push(dd[1])
      }
      return out
    }

    /** robots.txt 专用：几乎总是 text/plain，不能走 fetchCrawlPage 的 content-type 门槛。失败=空文本=放行。 */
    async function fetchRobotsText(origin, budgetEnd) {
      if (typeof fetch !== 'function') return ''
      const ctrl = new AbortController()
      const timer = setTimeout(() => { ctrl.abort() }, CRAWL_PER_REQ_TIMEOUT)
      try {
        if (Date.now() > budgetEnd) return ''
        const res = await fetch(origin + '/robots.txt', { redirect: 'error', signal: ctrl.signal })
        if (!(res.status >= 200 && res.status < 300)) { await res.text().catch(() => {}); return '' }
        return (await res.text()).slice(0, 64 * 1024)
      } catch { return '' } finally { clearTimeout(timer) }
    }

    async function fetchCrawlPage(url, budgetEnd) {
      if (typeof fetch !== 'function') throw new Error('宿主 Node 无 fetch 全局，网页抓取不可用（其余功能不受影响）')
      let cur = url
      for (let hop = 0; hop <= 3; hop++) {
        if (Date.now() > budgetEnd) return { error: 'timeout-budget' }
        const ctrl = new AbortController()
        const timer = setTimeout(() => { ctrl.abort() }, CRAWL_PER_REQ_TIMEOUT)
        let res, buf
        try {
          res = await fetch(cur, { redirect: 'error', headers: { 'user-agent': 'Mozilla/5.0 (compatible; study-plugin-web-course/1)' }, signal: ctrl.signal })
          if ([301, 302, 303, 307, 308].includes(res.status)) {
            const loc = res.headers.get('location')
            await res.text().catch(() => {})
            if (!loc || hop === 3) return { error: 'redirect-out-of-scope' }
            let next
            try { next = new URL(loc, cur) } catch { return { error: 'bad-redirect' } }
            if (next.protocol !== 'http:' && next.protocol !== 'https:') return { error: 'bad-redirect' }
            if (!sameSite(next.hostname, new URL(cur).hostname)) return { error: 'redirect-out-of-scope' }
            cur = next.toString()
            continue
          }
        } catch (e) {
          return { error: 'network: ' + errText(e) }
        } finally {
          clearTimeout(timer)
        }
        if (!(res.status >= 200 && res.status < 300)) return { error: 'HTTP ' + res.status }
        const ct = String(res.headers.get('content-type') || '')
        if (!/text\/html/i.test(ct)) return { notHtml: true, status: res.status }
        buf = Buffer.from(await res.arrayBuffer())
        if (buf.length > CRAWL_MAX_BYTES_PER_PAGE) return { truncated: true, status: res.status, html: buf.subarray(0, CRAWL_MAX_BYTES_PER_PAGE).toString('utf8'), finalUrl: cur }
        return { status: res.status, html: buf.toString('utf8'), finalUrl: cur }
      }
      return { error: 'redirect-out-of-scope' }
    }

    /** 抓取协程（fire-and-forget）：goal.webCrawl 由本函数独家维护，面板经轮询读进度。 */
    async function runWebCrawl(goalId, startUrl) {
      const finish = { goalId, startUrl }
      try {
        let g = await loadGoal(goalId)
        if (!g) throw new Error('目标不存在')
        const absDir = await goalDir(g)
        const webDir = absDir + '/research/web'
        await fsp.rm(absDir + '/research', { recursive: true, force: true })   // 幂等重抓：整批重建
        await fsp.mkdir(webDir, { recursive: true })
        const budgetEnd = Date.now() + CRAWL_TOTAL_BUDGET
        const persist = async (patch) => {
          g = await loadGoal(goalId)
          if (!g) return
          g.webCrawl = Object.assign({}, g.webCrawl, patch)
          g.updatedAt = nowISO()
          await saveGoal(g)
        }
        const origin = new URL(startUrl).origin
        const disallow = parseRobotsDisallow(await fetchRobotsText(origin, budgetEnd))
        ...
        while (queue.length) {
          ...
          if (pages.length >= CRAWL_MAX_PAGES) {
            pages.push({ url: item.url, status: 'skipped', reason: 'page-limit' })
            continue
          }
          const norm = normalizeCrawlUrl(item.url)
          if (!norm || visited.has(norm)) continue
          visited.add(norm)
          if (Date.now() > budgetEnd) {
            pages.push({ url: norm, status: 'skipped', reason: 'timeout-budget' })
            continue
          }
          if (disallow.some((p) => new URL(norm).pathname.startsWith(p))) {
            pages.push({ url: norm, status: 'blocked', reason: 'robots' })
            continue
          }
          let rec = { url: norm, depth: item.depth }
          const r = await fetchCrawlPage(norm, budgetEnd)
          if (r.notHtml) {
            rec = Object.assign(rec, { status: 'skipped', reason: 'not-html' })
          } else if (r.error) {
            rec = Object.assign(rec, { status: 'failed', reason: r.error })
          } else {
            const parsed = parseHtmlForCrawl(r.html, r.finalUrl)
            const fname = pad2(pages.length + 1) + '-' + slug(parsed.title || new URL(norm).pathname.replace(/\/+$/, '').split('/').pop() || 'page') + '.txt'
            await fsp.writeFile(webDir + '/' + fname, parsed.text, 'utf8')
            rec = Object.assign(rec, { status: 'ok', file: fname, title: parsed.title || undefined, chars: parsed.text.length, low: parsed.text.length < CRAWL_MIN_CHARS, truncated: r.truncated || undefined })
            okCount++
            if (item.depth < CRAWL_MAX_DEPTH) {
              for (const l of parsed.links) {
                let lu
                try { lu = new URL(l) } catch { continue }
                if (!sameSite(lu.hostname, new URL(r.finalUrl).hostname)) continue
                const ext = urlExt(l)
                if (ext && ext !== 'html') continue
                const n2 = normalizeCrawlUrl(l)
                if (n2 && !visited.has(n2) && !queue.some((q) => normalizeCrawlUrl(q.url) === n2)) queue.push({ url: n2, depth: item.depth + 1 })
              }
            }
          }
          pages.push(rec)
          await persist({ pages: pages.slice(), status: 'running' })
          await fsp.writeFile(webDir + '/manifest.json', JSON.stringify({ crawledAt: nowISO(), startUrl, pages }, null, 2), 'utf8')
        }
        await persist({
          status: okCount > 0 ? 'done' : 'failed',
          finishedAt: nowISO(),
          pages,
          error: okCount > 0 ? undefined : '全部页面抓取失败（详见预览清单）'
        })
        // 循环内的逐页 manifest 写发生在 push 之前；收尾必须补一次，否则 page-limit 等尾部落不进包
        await fsp.writeFile(webDir + '/manifest.json', JSON.stringify({ crawledAt: nowISO(), startUrl, pages }, null, 2), 'utf8')
        if (!g.webSources) g.webSources = []
        finish.ok = true
      } catch (e) {
        finish.error = errText(e)
        try {
          const g2 = await loadGoal(goalId)
          if (g2) {
            g2.webCrawl = Object.assign({}, g2.webCrawl, { status: 'failed', finishedAt: nowISO(), error: finish.error })
            g2.status = g2.webCrawl && (g2.webCrawl.pages || []).length ? 'researching' : g2.status
            g2.updatedAt = nowISO()
            await saveGoal(g2)
          }
        } catch {}
      } finally {
        webCrawlActive.delete(goalId)
      }
      return finish
    }
    ...
    async function approveGoal(g) {
      ...
      g.chapters = g.draft.chapters.map((c) => ({
        ...
        env_baseline: c.env_baseline, depends_on: c.depends_on, project_thread: c.project_thread, gap_notes: c.gap_notes,
    ...
    async function listSummary() {
      ...
      for (const row of (idx.goals || [])) {
        ...
        try { await adoptTestFiles(g) } catch (e) {}
        const testView = (list) => (list || []).map((t) => ({
          n: t.n,
          status: t.status === 'ready' ? 'ready' : (t.status || 'generating'),
          file: t.file,
          sessionId: t.sessionId || undefined,
          createdAt: t.createdAt || undefined
        }))
        const mistakesCount = async (p) => { try { const raw = await readTextFile(p); return raw === undefined ? 0 : countMistakes(raw) } catch (e) { return 0 } }
        const chMistakes = {}
        for (const c of (g.chapters || [])) chMistakes[c.index] = await mistakesCount(row.path + '/chapters/' + chapterMistakesFile(c.index))
        out.push({
          id: g.id,
          title: g.title,
          ...
          draft: g.draft ? {
            ...
            chapters: Array.isArray(g.draft.chapters) ? g.draft.chapters.map((c) => ({ index: c.index, title: c.title, summary: c.summary, est_hours: c.est_hours === undefined ? null : c.est_hours, focus_points: c.focus_points || [], env_baseline: c.env_baseline, depends_on: c.depends_on || [], project_thread: c.project_thread, gap_notes: c.gap_notes })) : []
          } : null,
          chapters: Array.isArray(g.chapters) ? g.chapters.map((c) => ({
            ...
            test: testView(c.tests),
            mistakes: chMistakes[c.index] || 0
          })) : [],
          goalTests: testView(g.goalTests),
          goalMistakes: await mistakesCount(row.path + '/' + GOAL_MISTAKES_FILE),
          web: (() => {
            const wc = g.webCrawl
            if (!wc) return undefined
            const pages = wc.pages || []
            const n = (st) => pages.filter((p) => p.status === st).length
            return {
              status: wc.status,
              startedAt: wc.startedAt,
              finishedAt: wc.finishedAt,
              error: wc.error,
              startUrl: (g.webSources && g.webSources[0] && g.webSources[0].url) || undefined,
              ok: n('ok'), failed: n('failed'), skipped: n('skipped'), blocked: n('blocked'),
              total: pages.length
            }
          })(),
          path: row.path
    ...
    handlers['study.startResearch'] = async (args) => {
      ...
      const absDir = await goalDir(g)
      try {
        injectToSession(sessionId, await researchInstruction(g, absDir))
      } catch (e) {
        return { ok: false, error: errText(e) }
      }
    ...
    handlers['study.retryResearch'] = async (args) => {
      ...
      try {
        injectToSession(g.sessionId, await retryInstruction(g, absDir, reason))
      } catch (e) {
        return { ok: false, error: errText(e) }
      }
    ...
    const buildGenInstruction = async (g, ch, absDir) => {
      const dirUrl = absDir.replace(/\\/g, '/')
      const absCh = dirUrl + '/chapters/' + ch.file
      const prefs = await readTeachingPrefs()
      const all = (g.chapters || []).slice().sort((a, b) => a.index - b.index)
      const prior = all.filter((c) => c.index < ch.index && (c.status === 'ready' || c.status === 'done'))
      const toc = all.map((c) => '第' + c.index + '章 ' + c.title + ((c.focus_points || []).length ? '(' + c.focus_points.join('、') + ')' : '')).join(' → ')
      const overview = (g.draft && g.draft.overview) ? String(g.draft.overview) : ''
      let s = '请撰写第 ' + ch.index + ' 章「' + ch.title + '」的讲义。\n'
        + '定位: ' + (ch.summary || '') + '；核心知识点: ' + (ch.focus_points || []).join('、') + '；学习者目标: ' + (g.target_level || '') + '。\n'
        + (g.requirements ? '风格要求: ' + g.requirements + '\n' : '')
        + prefsSection(prefs)
        + (overview ? '全课程总览(先总之总，本章的"分"必须挂在这张框架上): ' + overview + '\n' : '')
        + '章次地图: ' + toc + '\n'
      if (prior.length) {
        s += '前置章讲义(动笔前必须用 read 工具逐份读完): ' + prior.map((c) => '第' + c.index + '章=' + dirUrl + '/chapters/' + c.file).join('；') + '。\n'
          + '读前置章的三个目的: ① 环境与工程线——本章继续建立在第1章确立的统一工程与 git 仓库之上(' + (ch.project_thread || '增量方式见前置章讲义的分支动线') + ')，严禁重搭环境或另起工程；② 欠账清偿——前置章中标注「作黑盒使用，第 ' + ch.index + ' 章详述」或"留待后续"且属本章职责的概念，在本章「承接与补完」小节先补完再讲新内容，并在前置章原标注处用 edit 追加一行「→ 已在第 ' + ch.index + ' 章讲清」；③ 衔接核对——前置章小结与本章知识点的交叠处，不重复讲解也不留断档。\n'
          + (ch.depends_on && ch.depends_on.length ? '本章申报的依赖章: ' + ch.depends_on.join('、') + '（优先核对这些章的欠账标注）。\n' : '')
      } else {
        s += '本章是全课程环境与工程线的起点(第1章): 讲义必须完成工具链安装、最小可运行工程(如 HelloWorld)、git 仓库初始化(git init, 主分支=全课程基线, 打完基线提交)，并写明工程位置与后续章统一遵循的分支约定——每章从上一章合并点 git checkout -b chapter/NN-主题 独立分支开发，章末验证后合并回主分支，禁止直接在主分支开发。' + ((ch.env_baseline) ? '环境基线: ' + ch.env_baseline + '\n' : '\n')
      }
      s += '把完整 Markdown 讲义写入文件 ' + absCh + '（若工具允许写文件）；中文。若无写文件工具，请在回复中输出完整 Markdown。\n'
        + '讲义结构(先总后分, 固定顺序): ①标题 → ②本章概览(总): 本章在全课程框架中的位置与作用 + 本章抽象框架(优先用 mermaid 框架图) + 分节地图 → ③承接与补完: ' + (prior.length ? '统一工程接力与本章分支动线(checkout -b chapter/' + pad2(ch.index) + '-主题, 从上一章合并点起)，清偿前置章黑盒欠账' : '本章确立环境基线与分支约定(git init+主分支基线提交)') + ' → ④正文分节(每个小节同样先给结论/框架再逐点展开; 示例/类比/常见误区随文) → ⑤动手练习(基于统一工程增量开发: 写明打开哪个工程、在哪个分支、文件加在哪、如何运行; 严禁另起炉灶) → ⑥本章小结(回收总览, 逐条对应正文)。\n'
        + '硬约束: 知识闭环——正文/示例/练习中出现的每个概念，必须本章前文或既往章节已讲过；确需借用后续章节概念时标注「作黑盒使用，第 N 章详述」；小结严禁出现正文未讲解的知识点。排版——公式一律 LaTeX(行内 $…$、独立 $$…$$)，禁止 ASCII 拼写伪公式；结构/流程/时序/依赖图优先用 mermaid 代码块，一图配一句文字说明(图不替代正文)。\n'
        + '完稿前自检(不达标先修改再落盘): 小结逐条能在正文找到对应讲解；练习全部基于统一工程与本章分支；无来源不明的概念；公式均为 LaTeX、结构图均为 mermaid。\n'
        + prefsWriteBackHint
      return s
    }

    // ── D36: 测试单元指令 —— 出卷（蓝图先行 + 错题占额度）与陪练（逐题讲评 + 记错题） ──
    /** spec: { scope:'chapter'|'goal', ch?, entry:{n,file,requirements?} } */
    const buildTestInstruction = async (g, absDir, spec) => {
      const dirUrl = absDir.replace(/\\/g, '/')
      const prefs = await readTeachingPrefs()
      const isChapter = spec.scope === 'chapter'
      const label = isChapter
        ? '第 ' + spec.ch.index + ' 章「' + spec.ch.title + '」的单元讲义测试'
        : '目标测试（覆盖全课程）'
      const basis = isChapter
        ? '本章讲义: ' + dirUrl + '/chapters/' + spec.ch.file
        : '全课程总览: ' + ((g.draft && g.draft.overview) || '') + '\n全部讲义(出卷前用 read 逐份读完): '
          + (g.chapters || []).filter((c) => c.status === 'ready' || c.status === 'done').map((c) => dirUrl + '/chapters/' + c.file).join('；')
      const testPath = dirUrl + (isChapter ? '/chapters/' : '/') + spec.entry.file
      const mistakesPath = dirUrl + (isChapter ? '/chapters/' + chapterMistakesFile(spec.ch.index) : '/' + GOAL_MISTAKES_FILE)
      const extra = String(spec.entry.requirements || spec.requirements || '').trim()
      return '你是出卷 AI。请为学习目标「' + g.topic + '」（目标水平: ' + (g.target_level || '未说明') + '）生成一份新测试: ' + label + '。\n'
        + (g.requirements ? '课程风格要求: ' + g.requirements + '\n' : '')
        + (extra ? '用户对本卷的附加要求(优先，可指定题型配比/侧重模块/难度): ' + extra + '\n' : '')
        + prefsSection(prefs)
        + '出题依据: ' + basis + '\n'
        + '## 工作规程（严格按序，不可跳过）\n'
        + '1) 只先给出卷蓝图（题型×题量×模块配比 + 分值结构），**先不写任何文件**，末尾问一句:「按此蓝图出卷？确认或说调整点。」若上方附加要求已给全题型与题量，复述蓝图后直接执行到第 3 步。\n'
        + '2) 等学习者在本会话中确认或调整，以最新确认的蓝图为准。\n'
        + '3) 用 read 读取错题本 ' + mistakesPath + '（文件不存在则跳过错题匹配），按考点/模块归类条目。\n'
        + '4) 错题按额度匹配组卷: 蓝图每个模块槽位至多取该模块错题数=槽位数（原样或轻微改编均可，优先标「重错」的条目）；错题少于槽位 ⇒ 缺口出新题；多于槽位 ⇒ 落选者留待下次；绝不超插蓝图外模块的错题。\n'
        + '5) 把完整试卷（Markdown）写入 ' + testPath + '：题目在前、答案在后——全部答案与解析集中在末尾「参考答案与解析」节；每题标注考点；错题重做题在题目标题行标【错题重做｜来源 <原条目>】。公式一律 LaTeX($…$/$$…$$)，结构图优先 mermaid。\n'
        + '6) 落盘后一句话回复:「已生成考卷(共 N 题，含错题重做 X 题)」，并提示到学习区展开测试折叠区点「▶ 打开」进入逐题陪练。\n'
        + mistakeRuleText(mistakesPath)
        + prefsWriteBackHint
    }

    /** spec: { scope:'chapter'|'goal', ch?, entry:{n,file} } —— 陪练会话由面板「▶ 打开」建立。 */
    const buildSparringInstruction = async (g, absDir, spec) => {
      const dirUrl = absDir.replace(/\\/g, '/')
      const prefs = await readTeachingPrefs()
      const isChapter = spec.scope === 'chapter'
      const label = isChapter
        ? '第 ' + spec.ch.index + ' 章「' + spec.ch.title + '」测试 ' + pad2(spec.entry.n)
        : '目标测试 ' + pad2(spec.entry.n)
      const testPath = dirUrl + (isChapter ? '/chapters/' : '/') + spec.entry.file
      const mistakesPath = dirUrl + (isChapter ? '/chapters/' + chapterMistakesFile(spec.ch.index) : '/' + GOAL_MISTAKES_FILE)
      return '你是陪练教练。学习目标「' + g.topic + '」——' + label + '，试卷文件: ' + testPath + '。\n'
        + prefsSection(prefs)
        + '## 陪练流程\n'
        + '1) 用 read 读完整试卷，逐题呈现（一次只出当前题与选项，不透露答案）；若本会话已做到中途，从未完成处继续。\n'
        + '2) 学习者答对: 一句点评，出下一题。\n'
        + '3) 学习者答错: 必须讲清三件事——①这个答案为什么错（易错点）②该题考点是什么 ③考点内容（基于讲义适当展开讲透）；随后按下方规则把该错题追加到错题本 ' + mistakesPath + '（来源字段写 ' + spec.entry.file + '；先 read 再写，避免与既有条目重复主题）。\n'
        + '4) 学习者答对【错题重做】题: 在错题本对应条目末尾追加一行「→ 已在 YYYY-MM-DD 重做答对」。\n'
        + '5) 全部完成: 汇报成绩（对/总）并列本次错考点清单，提示可用学习区 📝/🎯 或说「再来一份」按薄弱点生成新卷。\n'
        + mistakeRuleText(mistakesPath)
        + '中文交流。'
    }
    ...
    handlers['study.generateChapter'] = async (args) => {
      ...
      await saveGoal(g)
      const absDir = await goalDir(g)
      const instruction = await buildGenInstruction(g, ch, absDir)
      if (ch.sessionId) {
    ...
    handlers['study.continueChapter'] = async (args) => {
      ...
      await saveGoal(g)
      try {
        injectToSession(sessionId, await buildGenInstruction(g, ch, absDir))
      } catch (e) {
        ch.status = 'draft'
        g.updatedAt = nowISO()
        await saveGoal(g)
    ...
    handlers['study.startChapter'] = async (args) => {
      ...
      const instruction = '你是本章学习教练。这是学习目标「' + g.topic + '」的第 ' + ch.index + ' 章「' + ch.title + '」。\n'
        ...
        + '4) 学习者说「结束」时总结本章要点，并询问是否把问答要点写入 ' + qaFilePath + '。\n'
        + '5) 【偏好沉淀】你调整本章讲义的结构或写法时，必须先遵守上述用户教学偏好；若学习者在会话中提出希望长期改变讲义/课程的结构、组织方式、构建方法或规则(非一次性要求)，提炼成一句话规则，征得学习者同意后追加到 ' + PREFS_PATH + '（格式: - YYYY-MM-DD 规则；文件不存在则创建；重复/冲突条目合并改写而非堆叠；只记规则与结构习惯，不记具体课程内容）。\n'
        + '6) 【错题与本章测试】本章问答(含你的随堂设问)中学习者答错时，按下方规则把错题记入 ' + chDir + '/' + chapterMistakesFile(idx) + '（来源字段写「讲义问答」或当次测试文件名）。学习者要求测试/测验本章时你即出卷 AI，流程: ①只先给蓝图(题型×题量×模块配比)并问确认(要求已给全题型题量则复述后直接继续) → ②read 本章错题本按考点归类(不存在则跳过) → ③错题按额度入卷: 每模块槽位至多取该模块错题数=槽位数(优先重错)，缺口出新题，绝不超插，重做题标【错题重做｜来源】 → ④新卷写入 ' + chDir + '/' + pad2(idx) + '-test-XX.md（XX=01,02…取目录现有最大号+1，先 ls 确认，绝不复用覆盖；题目在前，末尾「参考答案与解析」节，每题标考点；公式 LaTeX、图 mermaid） → ⑤一句话汇报并提示到学习区本章「测试」折叠区点「▶ 打开」进入逐题陪练。答错即记错题，规则:\n'
        + mistakeRuleText(chDir + '/' + chapterMistakesFile(idx))
        + '中文交流。'
      try {
    ...
    // ── 网页课程源 RPC（v0.8.0，docs/design/web-course.md §3/§5）─────────────
    handlers['study.startCrawl'] = async (args) => {
      const g = await loadGoal(String(args && args.goalId))
      if (!g) return { ok: false, error: '目标不存在' }
      const goalId = String(args && args.goalId)
      if (webCrawlActive.has(goalId) || (g.webCrawl && g.webCrawl.status === 'running')) {
        return { ok: false, error: '该目标正在抓取中，请等本批完成后再发起' }
      }
      const startUrl = normalizeCrawlUrl(String((args && args.url) || '').trim())
      if (!startUrl) return { ok: false, error: 'URL 无效：请填写 http/https 完整网址' }
      g.webSources = [{ url: startUrl, title: '' }]
      g.webCrawl = { status: 'running', startedAt: nowISO(), startUrl, pages: [] }
      g.status = 'crawling'
      g.updatedAt = nowISO()
      await saveGoal(g)
      // fire-and-forget：协程独家维护 webCrawl；任何异常都不冒回 RPC。
      // 同步占位防竞态（协程体内的 delete 在 finally 兜底清理）。
      webCrawlActive.add(goalId)
      runWebCrawl(goalId, startUrl).then(async (r) => {
        const g2 = await loadGoal(goalId)
        if (g2 && g2.status === 'crawling') {
          g2.status = 'researching'
          g2.updatedAt = nowISO()
          await saveGoal(g2)
        }
        return r
    ...
    handlers['study.getCrawlPreview'] = async (args) => {
      const g = await loadGoal(String(args && args.goalId))
      if (!g) return { ok: false, error: '目标不存在' }
      if (!g.webCrawl) return { ok: true, crawled: false }
      const absDir = await goalDir(g)
      let fileCount = 0
      try { fileCount = (await fsp.readdir(absDir + '/research/web')).filter((f) => f.endsWith('.txt')).length } catch (e) {}
      return { ok: true, crawled: true, webCrawl: g.webCrawl, webSources: g.webSources || [], materialFiles: fileCount }
    }

    // 整课一次性生成：一条指令、按全章序推进、只写「待生成」章（regenerate=true 时全部章覆盖重写）。
    const buildAllChaptersInstruction = async (g, absDir, targets, regenerate) => {
      const dirUrl = absDir.replace(/\\/g, '/')
      const prefs = await readTeachingPrefs()
      const all = (g.chapters || []).slice().sort((a, b) => a.index - b.index)
      const toc = all.map((c) => '第' + c.index + '章 ' + c.title + ((c.focus_points || []).length ? '(' + c.focus_points.join('、') + ')' : '')).join(' → ')
      const overview = (g.draft && g.draft.overview) ? String(g.draft.overview) : ''
      const material = await webMaterialOf(g)
      const targetSet = new Set(targets.map((c) => c.index))
      let s = '你是课程讲义撰写 AI。请为学习目标「' + g.topic + '」（目标水平: ' + (g.target_level || '未说明') + '）一次性整理全部讲义（共 ' + all.length + ' 章）。\n'
        + (g.requirements ? '风格要求: ' + g.requirements + '\n' : '')
        + prefsSection(prefs)
        + (overview ? '全课程总览(先总之总，每章的"分"必须挂在这张框架上): ' + overview + '\n' : '')
        + '章次地图: ' + toc + '\n'
      if (material.length) {
        s += '网页课程源材料(先通读再动笔，讲义事实优先以此为依据; ' + (regenerate ? '' : '材料讲不到的点按各章 gap_notes 补全') + '): '
          + material.map((m2) => m2.abs + '(来源 ' + m2.url + ')').join('；') + '。\n'
      }
      s += regenerate
        ? '工作方式: 按第 1→' + all.length + ' 章的**全章顺序**推进，保持章间承接。对每一章：动笔前先用 read 工具读完它前面所有章节的讲义原文，再撰写并**覆盖写入**该章自己的讲义文件。\n'
        : '工作方式: 按第 1→' + all.length + ' 章的**全章顺序**推进，保持章间承接。若第 i 章已在下列"已就绪"清单中——**绝不重写其讲义文件**，仅作为前置阅读；若第 i 章待生成，动笔前先用 read 工具读完第 1..i-1 章中全部已就绪讲义原文，再撰写并写入该章自己的讲义文件。\n'
      s += '各章讲义文件: ' + all.map((c) => '第' + c.index + '章=' + dirUrl + '/chapters/' + c.file).join('；') + '。\n'
      s += '每章固定六段结构(先总后分): ①标题 → ②本章概览(总): 本章在全课程框架中的位置与作用 + 本章抽象框架(优先用 mermaid 框架图) + 分节地图 → ③承接与补完: 统一工程接力与本章分支动线(从上一章合并点 checkout -b chapter/NN-主题, 章末验证点+合并回主分支; 第1章则 git init 确立基线)，清偿前置章标注「作黑盒使用，本章详述」的欠账并在前置章原标注处 edit 追加「→ 已在第 N 章讲清」 → ④正文分节(每小节先给结论/框架再逐点展开; 示例/类比/常见误区随文) → ⑤动手练习(基于统一工程增量开发, 严禁另起炉灶) → ⑥本章小结(回收总览, 逐条对应正文)。\n'
        + '硬约束: 知识闭环——每个概念必须本章前文或既往章节已讲过；借用后续章节概念标注「作黑盒使用，第 N 章详述」；小结严禁出现正文未讲解的知识点；禁止直接在主分支开发。排版——公式一律 LaTeX($…$/$$…$$)，结构/流程/时序图优先 mermaid，一图配一句文字说明。\n'
        + '缺料补全: ' + (material.length
          ? '各章 gap_notes 列出的是网页材料覆盖不到的知识点——用你自己的知识补全(有 web_search 工具时可辅以至多 2 次检索)，并在对应小节末尾单独一行标注「> 📤 补充：非原始网页来源」。\n'
          : '若某知识点在既有讲义与你的依据中都找不到，用自身知识补全并在此前标注「(补充) 」。\n')
      const readyList = all.filter((c) => !targetSet.has(c.index))
      if (!regenerate && readyList.length) {
        s += '已就绪(勿重写，仅作前置阅读): ' + readyList.map((c) => '第' + c.index + '章=' + dirUrl + '/chapters/' + c.file).join('；') + '。\n'
      }
      s += '待你撰写落盘的章节(写完一章立即落盘再写下一章): ' + targets.map((c) => '第' + c.index + '章「' + c.title + '」→ ' + dirUrl + '/chapters/' + c.file +
        ((c.focus_points || []).length ? '；核心知识点: ' + c.focus_points.join('、') : '') +
        (c.summary ? '；定位: ' + c.summary : '') +
        ((c.gap_notes || []).length ? '；gap_notes: ' + c.gap_notes.join('｜') : '')).join('；') + '。\n'
      s += '全部完成后用一句话回复: 讲义已生成(本次 N 章)。' + prefsWriteBackHint
      return s
    }

    handlers['study.generateAllChapters'] = async (args) => {
      const g = await loadGoal(String(args && args.goalId))
      if (!g) return { ok: false, error: '目标不存在' }
      if (!Array.isArray(g.chapters) || !g.chapters.length) return { ok: false, error: '尚无章节：请先完成调研并批准草案' }
      const regenerate = args && args.regenerate === true
      let targets
      if (regenerate) {
        targets = g.chapters.slice()
      } else {
        const pending = g.chapters.filter((c) => (c.status || 'draft') === 'draft')
        if (!pending.length) {
          const allReady = g.chapters.every((c) => c.status === 'ready' || c.status === 'done')
          return { ok: false, error: allReady ? '全部章节讲义已就绪' : '没有处于「待生成」的章节（进行中的章节请分别用「继续生成」）' }
        }
        targets = pending
      }
      if (g.chapters.some((c) => c.status === 'generating')) return { ok: false, error: '有章节正在生成中，请先处理完再进行一次性生成' }
      if (!g.sessionId) return { ok: false, error: '尚未记录目标会话，请先点「📄 打开会话」' }
      const absDir = await goalDir(g)
      for (const c of targets) c.status = 'generating'
      g.updatedAt = nowISO()
      await saveGoal(g)
      try {
        injectToSession(g.sessionId, await buildAllChaptersInstruction(g, absDir, targets, regenerate))
      } catch (e) {
        for (const c of targets) c.status = 'draft'
        g.updatedAt = nowISO()
        await saveGoal(g)
        return { ok: false, error: '派发失败（会话未激活？）: ' + errText(e) }
      }
      return { ok: true, chapters: targets.length, indices: targets.map((c) => c.index), regenerate: regenerate || undefined }
    }

    // ── D36: 测试单元 RPC ────────────────────────────────────────────────────
    // 生成 = 向会话派发一份「新卷」指令并预占编号（generating）；编号在盘上真有文件时
    // 由 adoptTestFiles 确认（AI 只给蓝图未落盘就中断 ⇒ 条目被剔除/留在 generating，编号不烧死）。
    const nextTestN = (list) => Math.max(0, ...(list || []).map((t) => Number(t.n) || 0)) + 1

    handlers['study.generateChapterTest'] = async (args) => {
      const g = await loadGoal(String(args && args.goalId))
      if (!g) return { ok: false, error: '目标不存在' }
      const idx = Number(args && args.chapter_index)
      const ch = (g.chapters || []).find((c) => c.index === idx)
      if (!ch) return { ok: false, error: '章节不存在' }
      if (ch.status !== 'ready' && ch.status !== 'done') return { ok: false, error: '本章讲义未就绪，不能出卷' }
      const absDir = await goalDir(g)
      await adoptTestFiles(g)
      ch.tests = ch.tests || []
      const n = nextTestN(ch.tests)
      const entry = { n, file: chapterTestFile(idx, n), status: 'generating', createdAt: nowISO() }
      const extra = String((args && args.requirements) || '').trim()
      if (extra) entry.requirements = extra
      ch.tests.push(entry)
      g.updatedAt = nowISO()
      await saveGoal(g)
      const instruction = await buildTestInstruction(g, absDir, { scope: 'chapter', ch, entry })
      // 与 generateChapter 同款两级回落：优先本章会话（带讲义上下文），退而目标会话。
      let injected = ''
      for (const sid of [ch.sessionId, g.sessionId].filter(Boolean).map(String)) {
        try { injectToSession(sid, instruction); injected = sid; break } catch (e) {}
      }
      if (!injected) {
        ch.tests = ch.tests.filter((t) => t !== entry)
        g.updatedAt = nowISO()
        await saveGoal(g)
        return { ok: false, error: '没有可用会话执行出卷：请先打开本章会话（「▶ 开始学习」）或目标会话（「📄 打开会话」）后重试' }
      }
      return { ok: true, test_n: entry.n, file: entry.file, sessionId: injected }
    }

    handlers['study.generateGoalTest'] = async (args) => {
      const g = await loadGoal(String(args && args.goalId))
      if (!g) return { ok: false, error: '目标不存在' }
      if (['researching', 'draft_pending', 'research_failed', 'crawling'].includes(g.status)) {
        return { ok: false, error: '课程草案批准前没有可覆盖的全课程材料，不能出目标卷' }
      }
      if (!g.sessionId) return { ok: false, error: '尚未记录目标会话，请先点「📄 打开会话」' }
      const absDir = await goalDir(g)
      await adoptTestFiles(g)
      g.goalTests = g.goalTests || []
      const n = nextTestN(g.goalTests)
      const entry = { n, file: goalTestFile(n), status: 'generating', createdAt: nowISO() }
      const extra = String((args && args.requirements) || '').trim()
      if (extra) entry.requirements = extra
      g.goalTests.push(entry)
      g.updatedAt = nowISO()
      await saveGoal(g)
      try {
        injectToSession(String(g.sessionId), await buildTestInstruction(g, absDir, { scope: 'goal', entry }))
      } catch (e) {
        g.goalTests = g.goalTests.filter((t) => t !== entry)
        g.updatedAt = nowISO()
        await saveGoal(g)
        return { ok: false, error: '派发失败（目标会话未激活？）: ' + errText(e) + '——请先「📄 打开会话」再试' }
      }
      return { ok: true, test_n: n, file: entry.file }
    }

    /** 定位一条测试：带 chapter_index ⇒ 章测试，否则目标测试。返回 { scope, ch?, t } 或 { error }。 */
    function findTestEntry(g, args) {
      const n = Number(args && args.test_n)
      if (!Number.isInteger(n)) return { error: '缺少 test_n' }
      const cRaw = args && args.chapter_index
      if (cRaw !== undefined && cRaw !== null && String(cRaw).trim() !== '') {
        const ch = (g.chapters || []).find((c) => c.index === Number(cRaw))
        if (!ch) return { error: '章节不存在' }
        const t = (ch.tests || []).find((x) => Number(x.n) === n)
        if (!t) return { error: '该章没有测试 ' + pad2(n) }
        return { scope: 'chapter', ch, t }
      }
      const t = (g.goalTests || []).find((x) => Number(x.n) === n)
      if (!t) return { error: '没有目标测试 ' + pad2(n) }
      return { scope: 'goal', t }
    }

    handlers['study.recordTestSession'] = async (args) => {
      const g = await loadGoal(String(args && args.goalId))
      if (!g) return { ok: false, error: '目标不存在' }
      const r = findTestEntry(g, args)
      if (r.error) return { ok: false, error: r.error }
      r.t.sessionId = String((args && args.sessionId) || '')
      if (!r.t.sessionId) return { ok: false, error: '缺少 sessionId' }
      g.updatedAt = nowISO()
      await saveGoal(g)
      return { ok: true }
    }

    handlers['study.startTest'] = async (args) => {
      const g = await loadGoal(String(args && args.goalId))
      if (!g) return { ok: false, error: '目标不存在' }
      const absDir = await goalDir(g)
      await adoptTestFiles(g)
      // adoptTestFiles 会改写 goal.json，定位放在采纳之后拿的是最新状态
      const r = findTestEntry(g, args)
      if (r.error) return { ok: false, error: r.error }
      if (r.t.status !== 'ready') return { ok: false, error: '测试卷还没生成完毕（当前: ' + (r.t.status || 'generating') + '），等出卷会话落盘后再打开' }
      const sessionId = String((args && args.sessionId) || r.t.sessionId || '')
      if (!sessionId) return { ok: false, error: '缺少 sessionId' }
      r.t.sessionId = sessionId
      g.updatedAt = nowISO()
      await saveGoal(g)
      try {
        injectToSession(sessionId, await buildSparringInstruction(g, absDir, { scope: r.scope, ch: r.ch, entry: r.t }))
      } catch (e) {
        return { ok: false, error: errText(e) }
      }
      return { ok: true }
    ...
    // scope: chapter-test | goal-test | chapter-mistakes | goal-mistakes
    // 落点全部由 goalId+编号经 goal.json/固定命名反查，file 字段再过 basename 全等（同 readChapter）。
    handlers['study.readTestFile'] = async (args) => {
      const g = await loadGoal(String(args && args.goalId))
      if (!g) return { ok: false, error: '目标不存在' }
      const scope = String((args && args.scope) || '')
      const absDir = await goalDir(g)
      let file, title, inChapters = true
      if (scope === 'chapter-test' || scope === 'goal-test') {
        await adoptTestFiles(g)
        const r = findTestEntry(g, args)
        if (r.error) return { ok: false, error: r.error }
        file = r.t.file
        title = (r.scope === 'chapter' ? '第 ' + r.ch.index + ' 章测试 ' : '目标测试 ') + pad2(r.t.n) + ' · ' + (r.scope === 'chapter' ? r.ch.title : g.topic)
        inChapters = r.scope === 'chapter'
      } else if (scope === 'chapter-mistakes') {
        const ch = (g.chapters || []).find((c) => c.index === Number(args && args.chapter_index))
        if (!ch) return { ok: false, error: '章节不存在' }
        file = chapterMistakesFile(ch.index)
        title = '第 ' + ch.index + ' 章错题本'
      } else if (scope === 'goal-mistakes') {
        file = GOAL_MISTAKES_FILE
        title = '目标错题本'
        inChapters = false
      } else {
        return { ok: false, error: '未知 scope: ' + scope }
      }
      const fileName = path.basename(String(file || ''))
      if (!fileName || fileName !== file) return { ok: false, error: '测试文件名不合法' }
      const filePath = absDir.replace(/\\/g, '/') + (inChapters ? '/chapters/' : '/') + fileName
      const content = await readTextFile(filePath)
      if (content === undefined) return { ok: false, error: '文件还不存在' }
      return { ok: true, filePath, title, content }
    }

    // 删除一份测试：从 goal.json 摘除该卷 + 删测试文件 + 物理删除该卷的陪练会话目录。
    // 宿主这一版没有「删除会话」API（见 PROJECT.md：归档按 id 且不可逆、无解档），因此走
    // locate → 取 transcript 所在会话目录 → 确认目录里确有 transcript 再 rm -rf。这属于对宿主
    // 落盘的带外改动：左栏镜像可能要重启 DSH 才刷新（客户端会尽力 refresh()，拉不动则回落提示）。
    handlers['study.deleteTest'] = async (args) => {
      const g = await loadGoal(String(args && args.goalId))
      if (!g) return { ok: false, error: '目标不存在' }
      const absDir = await goalDir(g)
      await adoptTestFiles(g)  // 采纳会改写 goal.json，定位放其后拿最新状态
      const r = findTestEntry(g, args)
      if (r.error) return { ok: false, error: r.error }
      const inChapters = r.scope === 'chapter'
      const fileName = path.basename(String(r.t.file || ''))
      if (!fileName || fileName !== r.t.file) return { ok: false, error: '测试文件名不合法' }
      const filePath = absDir.replace(/\\/g, '/') + (inChapters ? '/chapters/' : '/') + fileName
      const list = inChapters ? r.ch.tests : g.goalTests
      const at = Array.isArray(list) ? list.indexOf(r.t) : -1
      if (at < 0) return { ok: false, error: '内部错误：未能定位测试条目' }
      // 先摘条目并落盘：即便后续删盘失败，面板也不会再列出这份卷、不会重复触发删除
      list.splice(at, 1)
      g.updatedAt = nowISO()
      await saveGoal(g)
      const notes = []
      try { await fsp.unlink(filePath) }
      catch (e) { if (!e || e.code !== 'ENOENT') notes.push('测试文件删除失败: ' + errText(e)) }
      const sessionId = r.t.sessionId ? String(r.t.sessionId) : ''
      let sessionRemoved = false
      if (sessionId) {
        try {
          const pp = portableDeps.persistence
          if (!pp || typeof pp.locate !== 'function') throw new Error('会话持久化服务不可用，无法删除会话')
          const transcriptPath = sessionTargetPath(absDir, sessionId)
          const sessDir = path.dirname(transcriptPath)
          if (sessDir && sessDir !== absDir && await findTranscriptName(sessDir)) {
            await fsp.rm(sessDir, { recursive: true, force: true })
            sessionRemoved = true
          } else {
            notes.push('未找到该会话的落盘目录（可能从未真正产生消息），已跳过')
          }
        } catch (e) { notes.push('会话删除失败: ' + errText(e)) }
      }
      return stripUndefined({
        ok: true, goalId: String(args.goalId), test_n: r.t.n, scope: r.scope,
        sessionId: sessionId || undefined, sessionRemoved: sessionRemoved || undefined,
    ...
    async function buildGoalBundle(goalId) {
      ...
      // 全局教学偏好随包旅行（用户资产；导入侧做行级并集合并，见 study.importGoal）
      try {
        const prefsBuf = await fsp.readFile(PREFS_PATH)
        entries.push({ name: 'meta/teaching-prefs.md', data: prefsBuf, mtime: (await fsp.stat(PREFS_PATH)).mtime })
      } catch { /* 尚无偏好文件：不产生条目 */ }
    ...
    handlers['study.importGoal'] = async (args) => {
      ...
      try {
        ...
        // 1b) 全局教学偏好：行级并集合并（习惯是用户资产，多机/多包只增不丢；本机行永远保留）
        const prefsPkg = entries.get('meta/teaching-prefs.md')
        if (prefsPkg !== undefined) {
          const merged = P.mergePrefLines((await readTextFile(PREFS_PATH)) || '', prefsPkg.toString('utf8'))
          if (merged !== null) await stage(PREFS_PATH, Buffer.from(merged, 'utf8'))
        }
    ...
    /** 从固定仓删除某个已推送目标：删掉 study-goals/<rgid>/ 目录下的全部文件（本地目标与各设备账本一律不动）。
     *  不可逆 ⇒ 需 confirm=true（面板内联二次确认后带上）；未确认只回 needChoice 亮出将被删的内容规模。 */
    handlers['study.syncDeleteRemote'] = async (args) => {
      try {
        const a0 = args || {}
        const { cfg, err } = await requireBind()
        if (err) return err
        const rgid = String(a0.remoteGoalId || '').trim()
        if (!/^[A-Za-z0-9_-]+$/.test(rgid)) return { ok: false, error: '非法 remoteGoalId' }
        const base = '/repos/' + cfg.repo.owner + '/' + cfg.repo.name
        const refQ = '?ref=' + encodeURIComponent(cfg.repo.branch)
        const dirPath = goalPathOf(rgid)
        // 递归收集该目录下的全部文件（正常只有 bundle.zip + meta；防有人手动塞了子目录）
        const collect = async (rel) => {
          const r = await ghApi(cfg, 'GET', base + '/contents/' + rel + refQ)
          if (r.status === 404) return []
          if (!(r.status >= 200 && r.status < 300)) { await markInvalid(cfg, new GhError('', r)); throw new GhError('列目录 ' + rel, r) }
          const items = Array.isArray(r.data) ? r.data : (r.data && r.data.type === 'file' ? [r.data] : [])
          const files = []
          for (const it of items) {
            if (!it) continue
            if (it.type === 'dir') files.push(...await collect(rel + '/' + it.name))
            else if (it.type === 'file') files.push({ path: String(it.path), sha: String(it.sha) })
          }
          return files
        }
        const files = await collect(dirPath)
        if (files.length === 0) return stripUndefined({ ok: true, noop: true, deleted: 0, remoteGoalId: rgid, note: '仓库 ' + dirPath + '/ 没有可删的文件（可能已被其它设备删除）' })
        const confirm = a0.confirm === true || String(a0.confirm).toLowerCase() === 'true'
        if (!confirm) {
          const rw = await fetchRemoteMeta(cfg, rgid).catch(() => null)
          return stripUndefined({ ok: false, needChoice: true, error: '删除远端目标不可逆：会从仓库 ' + cfg.repo.fullName + ' 的 ' + dirPath + '/ 下删除 ' + files.length + ' 个文件（该目标已推送的全部数据），本地目标与其它设备不受影响。确认请再点一次「确认删除」', hint: '带 confirm=true 重调', files: files.map((f) => f.path), remote: rw && rw.meta ? { title: rw.meta.title, exportedAt: rw.meta.exportedAt, deviceId: rw.meta.deviceId, bytes: Number(rw.meta.zipBytes || 0) } : undefined })
        }
        const deleted = []
        for (const f of files) {
          const r = await ghApi(cfg, 'DELETE', base + '/contents/' + f.path, { message: 'study-plugin: delete remote goal ' + rgid, sha: f.sha, branch: cfg.repo.branch })
          if (r.status === 404 || r.status === 409) continue   // 已被并发删除 / sha 已变 ⇒ 视作已清理，跳过
          if (!(r.status >= 200 && r.status < 300)) { await markInvalid(cfg, new GhError('', r)); throw new GhError('删除 ' + f.path, r) }
          deleted.push(f.path)
        }
        return stripUndefined({ ok: true, deleted: deleted.length, deletedFiles: deleted, repo: cfg.repo.fullName, prefix: GH_PREFIX, remoteGoalId: rgid, note: '已从仓库删除该目标；本机目标与本地文件未改动，各设备下次检查会转为「☁ 远端还没有」' })
    ...
    async function chatStatus() {
      ...
      for (const row of (idx.goals || [])) {
        ...
        let testsReady = 0, mistakesTotal = 0
        for (const c of chapters) {
          testsReady += (c.tests || []).filter((t) => t.status === 'ready').length
          try { mistakesTotal += countMistakes(await readTextFile(row.path + '/chapters/' + chapterMistakesFile(c.index)) || '') } catch (e) {}
        }
        const goalTestsReady = (g.goalTests || []).filter((t) => t.status === 'ready').length
        try { mistakesTotal += countMistakes(await readTextFile(row.path + '/' + GOAL_MISTAKES_FILE) || '') } catch (e) {}
        ...
        out.push({
          ...
          tests_ready: testsReady,
          goal_tests_ready: goalTestsReady,
    ...
    async function chatResearch(args) {
      ...
      await clearDraftFile(absDir)
      const reason = (g.draft && g.draft.reject_reason) ? String(g.draft.reject_reason) : ''
      const text = reason ? await retryInstruction(g, absDir, reason) : await researchInstruction(g, absDir)
      try {
    ...
    // 外部会话的测试派发口：goal 与章节必须由用户明确 ⇒ 缺 goal_id 一律拒绝，不猜。
    // 带 chapter_index ⇒ 单元讲义测试；不带 ⇒ 目标测试。蓝图确认发生在被派发的会话里。
    async function chatTestGenerate(args) {
      const goalId = String((args && args.goal_id) || '').trim()
      if (!goalId) return { ok: false, error: '缺少 goal_id：请先调用 study_plan_status 把目标列给学习者确认选择，不要猜目标' }
      const cRaw = args && args.chapter_index
      const chIdx = cRaw === undefined || cRaw === null || String(cRaw).trim() === '' ? undefined : Number(cRaw)
      const requirements = String((args && args.requirements) || '').trim()
      const r = chIdx !== undefined
        ? await handlers['study.generateChapterTest']({ goalId, chapter_index: chIdx, requirements })
        : await handlers['study.generateGoalTest']({ goalId, requirements })
      if (!r || r.ok !== true) return r || { ok: false, error: '派发失败' }
      return {
        ok: true, goal_id: goalId, chapter_index: chIdx, test_n: r.test_n, file: r.file,
        message: '出卷已派发到' + (chIdx !== undefined ? '第 ' + chIdx + ' 章所属会话' : '目标会话')
          + '：蓝图确认与落卷都在该会话内进行——请学习者打开它（学习区「打开会话/开始学习」）确认蓝图；进度在学习区折叠区可见'
      }
    }
    ...
    const handleStudyFile = async (req, res) => {
      ...
      const serveDoc = (title, content, abs) => {
        res.writeHead(200, {
          'content-type': 'text/html; charset=utf-8',
          'content-security-policy': "default-src 'none'; style-src 'unsafe-inline'; img-src data: https:",
          'x-content-type-options': 'nosniff'
        })
        res.end('<!DOCTYPE html><html lang="zh"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>' + escapeHtml(title || '') +
      ...
      try {
        ...
        // D36: scope 参数 ⇒ 测试卷/错题本兜底页。落点仍由 readTestFile 从 goal.json 反查，
        // 本路由从不接受客户端给出的路径。
        const scope = String(u.searchParams.get('scope') || '')
        if (scope) {
          const r = await handlers['study.readTestFile']({
            goalId, scope,
            chapter_index: chapterParam === null ? undefined : Number(chapterParam),
            test_n: u.searchParams.get('test')
          })
          if (!r || r.ok !== true) return fail(404, (r && r.error) || 'test file not found')
          return serveDoc(r.title, r.content, r.filePath)
        }
    ...
    sctx.effect(() => {
      ...
      const specs = [
        ...
        ['study_test_generate',
          '派发一份新测试的生成（每次调用追加一份新卷，不覆盖旧卷）: 带 chapter_index=该章的单元讲义测试（讲义须已就绪）; 不带=目标测试（覆盖全课程）。注意: 学习者在某目标/章节会话内直接要卷时不要调用本工具（那会话自带上下文可直接出卷）; 跨会话或外部会话提出时才用。目标或章节不确定时先调用 study_plan_status 列给学习者确认，严禁猜 goal_id。蓝图（题型×题量×模块配比）确认发生在被派发的会话内。',
          {
            goal_id: { type: 'string', required: true, description: '目标 id（study_plan_status 可列出）' },
            chapter_index: { type: 'string', description: '章节号（整数字符串）；省略则派发目标测试' },
            requirements: { type: 'string', description: '用户对本卷的附加要求（题型配比/侧重模块/难度），可省略' }
          }, chatTestGenerate],
        ['study_goal_export',
    ...
    const README_LINES = [
      ...
      '- <goal>/goal.json —— 目标状态机主文件（status/draft/chapters[含 tests]/sessionId/workspaceId/goalTests）',
      ...
      '- <goal>/chapters/NN-test-XX.md —— 第 NN 章的第 XX 份单元讲义测试（每点一次生成新增一份，AI 落卷后轮询采纳为「试卷就绪」；题目在前，末尾集中「参考答案与解析」）',
      '- <goal>/chapters/NN-mistakes.md —— 第 NN 章错题本（讲义问答与测试陪练共用同一记录规则；每条错题一个 ## 块，只追加不删除，重错/重做答对都以追加标注体现）',
      '- <goal>/goal-test-XX.md、<goal>/goal-mistakes.md —— 目标级测试卷与错题本（覆盖全课程；与章维度同规则但各记各读）',
      '- _meta/teaching-prefs.md —— 用户教学偏好记忆体（只记讲义/课程的结构与规则类习惯，不记课程内容；由会话 AI 经确认后回写，随导出包跨机同步）',
      ...
      '测试: generating(已派发，含等你在会话内确认出卷蓝图) → ready(试卷就绪)；每点一次 📝/🎯 追加一份新卷，旧卷不覆盖。',
      '说明: 学习闭环 = 讲义 → 独立章节会话教学与多轮问答（要点沉淀 NN-qa.md）→ 单元/目标测试（专属陪练会话逐题作答）→ 答错讲解并记错题本 → 下次出卷按蓝图额度把错题纳入（优先重错、绝不超额度）。产品仍不设「上一章复习」环节。',
      '',
      '## 会话模型（重要）',
      '1. 每个学习目标有一个「目标总会话」：负责联网调研产出课程草案、以及把每章讲义写文件。打开方式: 学习区 → 该目标 → 📄 打开会话。建档不会自动调研：必须派发一次（学习区「▶ 开始调研」，或在聊天里说「开始调研」），目标会话里才会有活干。',
      '2. 每章一个「章节独立会话」：讲义就绪后点 📖（开始学习，图标钮，悬停看文字）创建并进入；本章讲解/问答都在该会话进行。AI 每次回答后会检查是否有值得补充到讲义的内容，有则询问是否回写。回写时：总结写入讲义（带链接），详细内容写入 NN-notes/ 目录下的独立文件，同主题复用同一文件。',
      '3. 主聊天（本会话）负责管理：查看进度、创建目标、批准/退回草案；学习行为发生在目标/章节会话里。',
      '4. 每份测试一个「陪练会话」：在测试折叠区点 ▶ 打开 创建/复用（sessionId 记在该卷条目里）。逐题呈现与作答都在该会话；答错 ⇒ AI 讲错误原因+考点+考点内容并追加错题到对应错题本；答对错题重做题 ⇒ 原条目追加「重做答对」标注。',
      '',
      '## 左侧「学习区」面板按钮',
      ...
      '- 📖 开始学习（纯图标，悬停出文字）: 为该章创建独立会话并进入（每章一个）；AI 每次回答后检查是否值得回写——总结入讲义（带链接），详细内容入 NN-notes/ 目录',
      '- 🎯 生成目标测试 / 📝 生成本章讲义测试（图标钮，悬停出文字）: 各派发一份新卷到目标会话/本章会话（讲义须就绪；没有本章会话会回落目标会话）。派发后面板转「生成中…」——出卷 AI 先在会话里只给出卷蓝图（题型×题量×模块配比）问你要不要调整，确认后按错题额度组卷并落盘',
      '- 测试折叠区（章下方「本章测试」/全部讲义下方「目标测试」，可独立折叠）: 每份卷一行，「▶ 打开」进该卷陪练会话并右侧页签开试卷；「📕」开错题本页签；折叠头显示份数与错题数',
      ...
      '',
      '## 导出 / 导入（可携化，为云同步铺垫）',
      '1. 导出包 = `study-goal-<goalId>-<时间戳>.zip`（同秒再导不会互相覆盖，自动加 -2/-3），内含: `manifest.json`（格式版本 v2/逐文件 sha256/每条会话的 remoteId+maxSeq+行数+blank/源机路径与 deviceId）、`goal/**`（目标目录整棵树）、`sessions/<会话id>/transcript.jsonl.zstd`（逐字节原文）、`attachments/objects/**`（内容寻址图片）、`workspace.json`（仅该目标登记信息）、`meta/teaching-prefs.md`（全局教学偏好，若存在；导入时与本机做**行级并集合并**——本机已有行不动、包内新行追加）。',
      ...
      '',
      '## 对话入口（在聊天里直接管理）',
      '可用工具: study_plan_status(进度总览与下一步，含测试份数与错题数) / study_plan_create(创建目标) / study_plan_research(发起调研) / study_plan_approve(批准草案) / study_plan_reject(退回草案) / study_test_generate(派发一份新测试: 带 chapter_index=章测试，不带=目标测试；goal 与章节必须先向学习者确认) / study_goal_export(导出 zip) / study_goal_import(预览与导入 zip，支持 mode=overwrite|merge|copy 与 force)。',
      '注: 该组工具由本常驻插件直接提供，DSH 启动即全局可用，任何模式无需激活。',
      '示例问法: 「看看我的学习进度」「帮我建一个学 XX 的计划（目标: …）」「开始调研」「草案可以，批准」「草案不行，XX 方向重做」「给第 3 章出一份测试」「来一份目标模拟卷，案例题占三成」。',
      '注: 在目标/章节会话里直接说「给我出一份测试」更快——那会话自带目标与讲义上下文，出卷 AI 会先与你确认蓝图；外部会话提出时才经 study_test_generate 派发，蓝图确认仍发生在被派发的会话里。',
