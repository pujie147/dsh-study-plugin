...
| chapters[] | 章节：index/title/summary/est_hours/focus_points/file/status/sessionId/qaFile，**tests[]**（v0.9.0 单元讲义测试：`{n,file,status:'generating'|'ready',sessionId?,createdAt,requirements?}`，编号只增不复用） |
| goalTests[] | 目标测试清单（v0.9.0）：条目形状与 ch.tests 一致，落点为目标根 `goal-test-XX.md` |
...
| reviewItems | 保留字段（旧版测验/错题残留，恒为空，不产生行为；v0.9.0 的测试体系不用它，见 D36） |
...
- 测试（v0.9.0，D36）：`generating（已派发，含等学习者在会话内确认出卷蓝图）→ ready（试卷就绪）`；每点一次 📝/🎯 追加一份新卷；采纳同讲义（`adoptTestFiles` 扫盘收编未登记的 `NN-test-XX.md` / `goal-test-XX.md`，>200 字判 ready）。
...
| study.generateChapterTest | goalId,chapter_index[,requirements] | 📝 派发一份该章**新卷**（v0.9.0，D36）：讲义须 ready/done；先 `adoptTestFiles` 扫盘防撞号，再预占 `{n,file,status:'generating'}` 条目；出卷指令（蓝图先行六步协议 + 本章错题本额度 + MISTAKE_RULE + teaching-prefs）注入本章会话（无则目标会话），双双不可用→报错并回滚条目 |
| study.generateGoalTest | goalId[,requirements] | 🎯 派发一份**新目标卷**（v0.9.0）：调研/草案待批/抓取中等状态一律拒；须有目标会话；其余同 generateChapterTest（依据=全部 ready 章 + overview，错题本=`goal-mistakes.md`） |
...
| `/study-file?goalId=&scope=&test=[&chapter=][&format=raw]` | 测试文档兜底页（v0.9.0）：scope=chapter-test / goal-test / chapter-mistakes / goal-mistakes，复用 `study.readTestFile` 的 handler（落点仍经 goal.json 反查 + basename 全等），渲染与讲义页同一 `serveDoc` 管线；无 scope 参数时行为与旧版逐字一致 |
...
| study_test_generate | 派发一份新测试（每次调用追加一份，不覆盖）：带 `chapter_index`=该章单元卷，不带=目标卷；**强制 `goal_id`**（缺了就报错指引先 `study_plan_status` 确认，严禁猜目标）。仅用于**外部**会话；学习者在目标/章节会话里要卷时不要调用（那会话指令自带出卷能力）。蓝图确认发生在被派发的会话内 |
...
| D36 | **测试单元（v0.9.0）＝ 多份测试 + 专属陪练会话，错题按蓝图额度纳入、绝不强插**：① 两类测试——**单元讲义测试**（每章讲义下独立折叠区，📝 派发）与**目标测试**（全部讲义下独立折叠区，🎯 派发），每次点击**追加一份新卷**（`ch.tests` / `g.goalTests` = `[{n,file,status,sessionId,createdAt,requirements?}]`，编号只增不复用），旧卷永不覆盖；② **蓝图先行**：出卷 AI 六步协议第一步只在会话里输出**出卷蓝图**（题型×题量×模块配比，如软考中级设计师）并请求确认，**确认前不写任何文件**；学习者需求里已给全题型/题量时复述后可直接继续；错题**按蓝图各模块槽位额度**从错题本拉入（优先带「重错」标注的条目，额度内取满、缺口出新题、超额留本），**永不超额度强插**；重做答对→原条目追加「→ 已在 YYYY-MM-DD 重做答对」，**永久留痕不销账**；③ **统一错题记录规则**（单一 `MISTAKE_RULE` 常量注入讲义/测试/陪练全部指令）：讲义问答答错与测试答错记**同一本**错题本、同一 ## 块格式，`来源` 字段区分维度；章维度读 `chapters/NN-mistakes.md`、目标维度只读 `goal-mistakes.md`（**各记各读，不做跨维聚合**）；④ **打开测试 = 「开始学习」同款闭环**：`openTest` 复用该卷已有会话或新建并 `study.recordTestSession` 记回，随后注入陪练指令（逐题呈现→答对简评→答错讲"错误原因+考点+考点内容"并记错题本→重做答对回标→成绩汇报），讲义经 better-sidebar 页签打开、缺席回落 `/study-file?scope=`（新 scope 分支：chapter-test/goal-test/chapter-mistakes/goal-mistakes，落点仍由 goalId+编号经 goal.json 反查 + basename 全等，旧章讲义路径逐字不变）；⑤ **在会话里要卷自带上下文**：`defineTool` 的 execute **拿不到调用方会话 id**（已核宿主工具契约，这是设计的关键约束），故"目标/章节会话内直接要卷"不靠工具反查，而是把能力文本注入调研/章节指令（`goalTestCapabilityText` / startChapter 第 6 条）；外部会话要卷必须走 `study_test_generate`，缺 `goal_id` 直接报错并指引先 `study_plan_status` 确认，**严禁猜目标**；⑥ 落卷采纳沿用 D3 file-as-truth：`adoptTestFiles` 扫盘收编未登记的 `NN-test-XX.md`/`goal-test-XX.md`（>200 字判 ready），派发前先扫（防盘上野文件撞号），注入失败**回滚删除预占条目**（编号不被失败烧掉） | 用户四轮迭代敲定的规格：两类测试+独立折叠、重复点击各生成新卷、UI 图标钮（🎯 在「打开会话」后、📝 在「开始学习」后，「开始学习」改纯图标悬停出文字）、AI 交互逐题作答并沉淀错题。关键取舍：错题**不能强行插入**（用户原话）⇒ 蓝图占额度模型，额度即硬上限；「3 次答对不再出现」被更正为**软优先**（重错标注影响排序，不硬排除）；generating 一档覆盖"等蓝图确认"（不新增状态、不设超时——AI 等确认期间面板就显示"生成中"）；同会话双击以最新确认的蓝图为准。测试：`smoke` 新增 D36 段（重复派发编号只增/注入失败回滚不烧号/adoptTestFiles 收编野文件/mistake 计数/recordTestSession 幂等/readTestFile 借道拒绝/讲义未 ready 拒出卷/study_test_generate 缺 goal_id 报错/`/study-file` scope 分支），`client.test` 新增图标钮与折叠区场景。**真机验收未做**：🎯/📝 派发→会话内蓝图确认→落卷→折叠区▶陪练→答错记错题→错题本页签→外部要卷先确认目标 |
...
- **测试单元（D36 / v0.9.0）的边界**：① 蓝图确认/额度组卷/逐题陪练全部是**提示词层约束**（承 D34/D35），插件不校验 AI 是否先问蓝图、是否超额度；generating 覆盖"等你在会话里确认蓝图"，**无超时**——不确认就一直显示"生成中"，可再点 📝/🎯 重发；② 错题计数按 `^## ` 块正则，AI 格式漂移（如漏写 `## `）会少计，面板错题数只是近似；③ 目标测试**只读** `goal-mistakes.md`，不与章错题聚合（用户拍板"各记各读"）；④ 「重做答对」是软优先不是硬排除——条目永久留在错题本，只影响下次组卷排序；⑤ 同一会话里连点两次派发出两份卷时，以**最新确认的蓝图**为准（插件不仲裁）；⑥ 测试卷/错题本在目标目录下 ⇒ 随导出与 GitHub 同步自动旅行。
