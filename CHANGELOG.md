...
### added
- **讲义生成增强（D34）**：修掉三个老毛病——第 1 章环境后面章节不接续、小结冒出没讲过的知识点、结构不按「先总后分」。三病同根：生成指令缺课程级上下文。全部改在**提示词层**（无硬校验，AI 不遵守时不报错；JavaEE 只是用户用来说明问题的例子，本次改动不涉及任何课程内容）：
  1. 章节生成指令固定六段**总分结构**：标题 → 本章概览(总) → 承接与补完 → 正文分节(每节先总后分、可递归) → 动手练习(统一工程增量) → 本章小结(只回收概览与正文讲过的内容)；
  2. **知识闭环硬约束** + 「作黑盒使用，第 N 章详述」欠账标记；生成第 N≥2 章前指令要求**用 read 工具逐份读完全部前置 ready 章讲义**（三个目的：接续环境与工程线 / 清偿挂到本章的欠账并就地改回「→ 已在第 N 章讲清」/ 衔接核对）；
  3. **git 分支工作流**：第 1 章 = 全课程环境与工程线起点（`git init`，主分支为全课程基线，**禁止直接在主分支开发**），每章从上一章合入点 `checkout -b chapter/NN-主题`、章末合回；
  4. 排版约定：公式一律 **LaTeX/KaTeX**（禁 ASCII 伪公式）、图一律 **mermaid**（一图配一句说明）。渲染侧已核到 better-sidebar@0.19.1 源码支持（preview 模式 mermaid 懒加载 chunk + `markdown-html.ts` 里的 KaTeX），本插件不动渲染、`/study-file` CSP 不放宽。
- **草案三个全可选字段**：`env_baseline`（环境基线）/ `depends_on`（依赖章号，采纳时清洗为 1..N 整数）/ `project_thread`（工程线约定），随草案采纳规范化、批准透传、`study.list` 回带。
- **教学偏好记忆体** `<workRoot>/_meta/teaching-prefs.md`：聊天里调整讲义结构/组织/构建规则时，AI 提炼一句话规则、**征得同意后**追加——**只记规则与结构习惯，不记具体课程内容**。调研/重调研/章节生成/章节教练四处指令都注入它，让习惯对后续所有章节生效。
- **偏好随包旅行**：导出 zip 新增 `meta/teaching-prefs.md` 条目（仅当文件存在）；导入按**行级并集**合并（纯函数 `P.mergePrefLines`：去空行、按行去重后追加，无新增即不动盘），GitHub 同步因此自动带上偏好变化。

### notes
- **只对新目标/新生成章节生效**：存量讲义不批量重生成；「重新生成讲义」按钮保持原样（调整讲义走 AI 聊天或手动改文件）。
- 测试：`smoke` 新增 D34 断言（四处指令内容、字段清洗与透传、偏好注入/沉淀、导出 meta 条目、导入并集重建）；`portable` 新增 §7b `mergePrefLines` 纯函数单测（32/32 绿）。`smoke` 导入段在本机仍被宿主 0.1.5-rc.2 漂移挡住（`cannot validate session`，经 `git stash` 验证未改动的 master 同样失败，非本轮回归），关键合并逻辑已由纯函数单测兜底。
- ⚠ **真机验收未做**：需新目标全链路（调研→批准→生成第 1 章→生成第 2 章）验证分支接续、概览首节、小结闭环、LaTeX/mermaid 呈现。

## [0.7.2] - 2026-09-18

### added
- **章节「打开讲义」（D33）**：点「📖 开始学习」（该功能**唯一入口**）在切换到章节会话后**自动打开本章讲义**，不再另设独立讲义按钮。落点分层，**不在左侧本面板内渲染**：
